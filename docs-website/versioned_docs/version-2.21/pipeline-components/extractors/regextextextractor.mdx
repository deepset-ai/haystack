...
extractor_explicit = RegexTextExtractor(regex_pattern=r"<answer>(.*?)</answer>", return_empty_on_no_match=False)
...
pipe.add_component(
    "extractor",
    RegexTextExtractor(regex_pattern=r"<summary>(.*?)</summary>", return_empty_on_no_match=False),
)
