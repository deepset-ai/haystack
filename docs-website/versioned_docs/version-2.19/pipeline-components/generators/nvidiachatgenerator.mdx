...
| **Output variables**                   | "replies": A list of [ChatMessage](/docs/chatmessage) objects                            |
...
`NvidiaChatGenerator` enables chat completions using NVIDIA's generative models via the NVIDIA API. It is compatible with the [ChatMessage](/docs/chatmessage) format for both input and output, ensuring seamless integration in chat-based pipelines.
...
To use this integration, you must have a NVIDIA API key. You can provide it with the `NVIDIA_API_KEY` environment variable or by using a [Secret](/docs/secret-management).

This generator supports [streaming](/docs/choosing-the-right-generator#streaming-support) responses from the LLM. To enable streaming, pass a callable to the `streaming_callback` parameter during initialization.
