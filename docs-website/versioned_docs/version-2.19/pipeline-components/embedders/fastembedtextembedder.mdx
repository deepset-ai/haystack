...
This component should be used to embed a simple string (such as a query) into a vector. For embedding lists of documents, use the [`FastembedDocumentEmbedder`](/docs/fastembeddocumentembedder), which enriches the document with the computed embedding, known as vector.
