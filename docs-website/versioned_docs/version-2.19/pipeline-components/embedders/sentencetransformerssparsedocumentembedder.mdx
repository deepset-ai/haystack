...
document_embedder = SentenceTransformersSparseDocumentEmbedder(token=Secret.from_token("<your-api-key>"))
```
...
```python
embedder = SentenceTransformersSparseDocumentEmbedder(model="prithivida/Splade_PP_en_v2", backend="onnx")
```
...
indexing_pipeline.add_component("sparse_document_embedder", SentenceTransformersSparseDocumentEmbedder())
