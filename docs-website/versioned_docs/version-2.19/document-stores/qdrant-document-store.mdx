...
- [`QdrantEmbeddingRetriever`](/docs/qdrantembeddingretriever): Retrieves documents from the `QdrantDocumentStore` based on their dense embeddings (vectors).
- [`QdrantSparseEmbeddingRetriever`](/docs/qdrantsparseembeddingretriever): Retrieves documents from the `QdrantDocumentStore` based on their sparse embeddings.
- [`QdrantHybridRetriever`](/docs/qdranthybridretriever): Retrieves documents from the `QdrantDocumentStore` based on both dense and sparse embeddings.
