...
| **API reference**            | [Toolset](https://docs.haystack.deepset.ai/reference/tools-api#toolset)      |
| **GitHub link**              | https://github.com/deepset-ai/haystack/blob/main/haystack/tools/toolset.py |

## Overview

A `Toolset` groups multiple Tool instances into a single manageable unit. It simplifies passing tools to components like Chat Generators, [`ToolInvoker`](/docs/toolinvoker), or [`Agent`](/docs/agent), and supports filtering, serialization, and reuse.
...
Here’s how to initialize `Toolset` with [Tool](tool.mdx). Alternatively, you can use [ComponentTool](/docs/componenttool) or [MCPTool](mcptool.mdx) in `Toolset` as Tool instances.
