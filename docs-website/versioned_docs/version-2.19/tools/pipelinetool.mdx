...
Use `PipelineTool` with the [Agent](/docs/agent) component. The `Agent` includes a `ToolInvoker` and your chosen ChatGenerator to execute tool calls and process tool results.
