...
`GitHubPRCreatorTool` wraps the [`GitHubPRCreator`](doc:githubprcreator) component, providing a tool interface for use in agent workflows and tool-based pipelines.
...
You can use `GitHubPRCreatorTool` with the [Agent](/docs/agent) component. The Agent will automatically invoke the tool when needed to create pull requests.
