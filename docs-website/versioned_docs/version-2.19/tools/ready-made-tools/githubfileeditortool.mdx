...
`GitHubFileEditorTool` wraps the [`GitHubFileEditor`](doc:githubfileeditor) component, providing a tool interface for use in agent workflows and tool-based pipelines.
...
You can use `GitHubFileEditorTool` with the [Agent](/docs/agent) component. The Agent will automatically invoke the tool when needed to edit files in GitHub repositories.
