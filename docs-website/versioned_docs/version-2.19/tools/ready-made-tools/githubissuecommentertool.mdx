...
`GitHubIssueCommenterTool` wraps the [`GitHubIssueCommenter`](doc:githubissuecommenter) component, providing a tool interface for use in agent workflows and tool-based pipelines.
...
You can use `GitHubIssueCommenterTool` with the [Agent](/docs/agent) component. The Agent will automatically invoke the tool when needed to post comments on GitHub issues.
