...
This example uses `mode="strict"` to return only the documents that match the query. See [Strict mode](#strict-mode) for how it differs from the default `fuzzy` mode.
