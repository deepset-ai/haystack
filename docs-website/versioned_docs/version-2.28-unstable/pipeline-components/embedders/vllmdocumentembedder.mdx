...
By default, if an embedding request fails, an exception is raised. Set `raise_on_failure=False` to log the error and continue processing the remaining documents instead.

### Instructions

Some embedding models require prepending the document text with an instruction to work better for retrieval. For example, if you use [google/embeddinggemma-300m](https://huggingface.co/google/embeddinggemma-300m), you should prefix your document with the following instruction: "title: none | text: ".
...
doc = Document(content="some text", meta={"topic": "relevant topic", "page_number": 18})
