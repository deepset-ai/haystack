...
async def convert():
    result = await converter.run_async(sources=["report.pdf", "notes.docx"])
    return result["documents"]


documents = asyncio.run(convert())
```
...
### Authenticated DoclingServe instance

If the DoclingServe server requires an API key, pass it via the `api_key` parameter or set the `DOCLING_SERVE_API_KEY` environment variable:
...
converter = DoclingServeConverter(base_url="https://my-docling-server.example.com")

# Or pass explicitly
converter = DoclingServeConverter(
    base_url="https://my-docling-server.example.com",
    api_key=Secret.from_token("my-api-key"),
)
```

### Custom conversion options

Pass DoclingServe conversion options to control OCR, table detection, and other parsing behavior:
...
converter = DoclingServeConverter(
    base_url="http://localhost:5001",
    convert_options={"do_ocr": True, "ocr_engine": "tesseract"},
)
