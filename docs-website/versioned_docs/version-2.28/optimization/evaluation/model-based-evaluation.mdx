...
| Supported metrics | Any metric from `ragas.metrics.collections` (such as `Faithfulness`, `AnswerRelevancy`, `ContextPrecision`, `ContextRecall`, `AspectCritique`, `SemanticSimilarity`, `FactualCorrectness`) | ANSWER_RELEVANCY, FAITHFULNESS, CONTEXTUAL_PRECISION, CONTEXTUAL_RECALL, CONTEXTUAL_RELEVANCE |
| Customizable prompt for response evaluation | ✅, with `AspectCritique` | ❌ |
