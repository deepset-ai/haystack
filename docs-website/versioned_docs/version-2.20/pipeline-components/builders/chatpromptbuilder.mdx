...
summary_template = [
    ChatMessage.from_user("Translate to {{ target_language }} and summarize. Context: {{ snippet }}; Summary:"),
]
...
res = pipe.run(
    data={"prompt_builder": {"template_variables": {"location": location, "language": language}, "template": messages}},
)
print(res)
```
...
res = pipe.run(
    data={"prompt_builder": {"template_variables": {"location": location, "day_count": "5"}, "template": messages}},
)
