...
`NvidiaRanker` ranks `Documents` based on semantic relevance to a specified query. It uses ranking models provided by [NVIDIA NIMs](https://ai.nvidia.com). If you don't set the `model` parameter, the hosted default `nv-rerank-qa-mistral-4b:1` is used.
