...
| **Mandatory init variables** | `model`: The model used through the STACKIT API  <br /> <br />`api_key`: A STACKIT API key. Can be set with `STACKIT_API_KEY` env var. |
...
The component needs a list of `ChatMessage` objects to run. `ChatMessage` is a data class that contains a message, a role (who generated the message, such as `user`, `assistant`, `system`, `tool`), and optional metadata. Find out more about it [ChatMessage documentation](../../concepts/data-classes/chatmessage.mdx).
