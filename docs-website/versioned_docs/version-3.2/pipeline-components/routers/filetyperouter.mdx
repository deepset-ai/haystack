...
| **Output variables** | `unclassified`: A list of uncategorized file paths or [byte streams](../../concepts/data-classes.mdx#bytestream)  <br /> <br />`failed`: A list of sources that could not be processed, for example a file path that doesn't exist  <br /> <br />`mime_types`: For example "text/plain", "text/html", "application/pdf", "text/markdown", "audio/x-wav", "image/jpeg": List of categorized file paths or byte streams |
...
Below is an example that uses the `FileTypeRouter` to route two file paths:
