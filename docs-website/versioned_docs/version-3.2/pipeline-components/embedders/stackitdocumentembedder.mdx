...
Set your preferred supported model with the `model` parameter when initializing the component. See the full list of all supported models on the [STACKIT website](https://docs.stackit.cloud/products/data-and-ai/ai-model-serving/basics/available-shared-models/).
...
Other optional parameters include `prefix` and `suffix` (added to each text before embedding), `batch_size`, `meta_fields_to_embed`, `embedding_separator`, and `dimensions`.

The component needs a list of documents as input to operate.
