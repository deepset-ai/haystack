...
| Azure DocumentDB | Document / NoSQL Database | Managed MongoDB-compatible document database (Microsoft Azure) built on the open source DocumentDB engine, with DiskANN, HNSW, and IVF vector search | Partial | Yes | Embedding, Full-text |
...
| Solr | Search Engine | Distributed search server (Apache Lucene-based) with BM25 + kNN dense vector search | Yes | Yes | BM25, Embedding, Hybrid |
...
- [Solr](../../document-stores/solrdocumentstore.mdx)
...
- [Azure DocumentDB](../../document-stores/azuredocumentdbdocumentstore.mdx)
