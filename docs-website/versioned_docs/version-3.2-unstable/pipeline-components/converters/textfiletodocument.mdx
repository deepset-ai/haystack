...
When you initialize the component, you can optionally set the default encoding of the text files through the `encoding` parameter. If you don't provide any value, the component uses `"utf-8-sig"` by default, which decodes plain UTF-8 identically and additionally strips a byte order mark (BOM) if the file has one. Note that if the encoding is specified in the metadata of an input ByteStream, it will override this parameter's setting.
