...
This example uses the `MetaFieldRanker` to rank two simple documents. When running the Ranker, you provide the `documents` and set the number of documents to rank using the `top_k` parameter.
