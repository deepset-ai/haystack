...
`VertexAIGeminiChatGenerator` supports Gemini models such as `gemini-3.8-flash`, `gemini-3.7-flash`, `gemini-2.5-pro`, and `gemini-2.5-flash`. Deprecated models like `gemini-2.0-flash` and `gemini-1.5-pro` are no longer available; see [Google's model versions page](https://cloud.google.com/vertex-ai/generative-ai/docs/learn/model-versions) for the migration path.
