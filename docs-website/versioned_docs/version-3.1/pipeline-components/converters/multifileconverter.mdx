...
| **Output variables** | `documents`: A list of converted documents  <br /> <br />`unclassified`: A list of file paths or byte streams whose MIME type isn't supported  <br /> <br />`failed`: A list of file paths or byte streams that couldn't be processed, for example a path that doesn't exist |
