...
In version 3.1, values selected by `content_key` must be strings or `null`. Numeric and boolean values raise a `ValueError`; convert them to strings in your JSON data or `jq_schema` before extraction. JSON `null` is preserved as `None`.

Check out the [API reference](/reference/converters-api#jsonconverter) for the full list of parameters.
