...
When using `content_key`, numbers and booleans become strings in `Document.content`. For example, `2026` becomes `"2026"` and `true` becomes `"True"`. JSON `null` remains `None`, and objects or arrays are skipped.
