...
- **Customize it.** Each entry point exposes a set of parameters for the choices specific to that agent, such as which LLMs to use. For everything else, such as adding tools or hooks, use [`clone()`](./agent.mdx#cloning-and-modifying-an-agent) on the returned agent.
