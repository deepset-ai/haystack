...
| **Mandatory run variables** | “messages”: A list of [`ChatMessage`](/docs/chatmessage)  objects |
| **Output variables** | "replies": A list of [`ChatMessage`](/docs/chatmessage) objects  <br /> <br />”meta”: A list of dictionaries with the metadata associated with each reply (such as token count, finish reason, and so on) |
...
`api_key` and using Haystack’s [secret management](/docs/secret-management).
...
The component needs a list of `ChatMessage` objects to run. `ChatMessage` is a data class that contains a message, a role (who generated the message, such as `user`, `assistant`, `system`, `function`), and optional metadata. Find out more about it [ChatMessage documentation](/docs/chatmessage).

### Streaming

This ChatGenerator supports [streaming](/docs/choosing-the-right-generator#streaming-support) the tokens from the LLM directly into the output. To do so, pass a function to the `streaming_callback` init parameter.
