...
To use this component with a specific file type, make sure you use the correct [Converter](doc:file-converters) before it. For example, to use `DocumentWriter` with Markdown files, use the `MarkdownToDocument` component before `DocumentWriter` in your indexing pipeline.
