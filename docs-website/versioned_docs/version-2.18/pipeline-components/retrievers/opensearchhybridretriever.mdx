...
description: "This is a [SuperComponent](/docs/supercomponents) that implements a Hybrid Retriever in a single component, relying on OpenSearch as the backend Document Store."
---

# OpenSearchHybridRetriever

This is a [SuperComponent](/docs/supercomponents) that implements a Hybrid Retriever in a single component, relying on OpenSearch as the backend Document Store.
...
1. **BM25 Retrieval**: A keyword-based search that uses the BM25 algorithm to find documents based on term frequency and inverse document frequency. It's based on the [`OpenSearchBM25Retriever`](doc:opensearchbm25retriever) component and is suitable for traditional keyword-based search.
