...
|  |  |
| --- | --- |
| **Most common position in a pipeline** | In query pipelines:   In a RAG pipeline, before a [`PromptBuilder`](/docs/promptbuilder)   In a semantic search pipeline, as the last component   In an extractive QA pipeline, before an [`ExtractiveReader`](/docs/extractivereader)  |
