...
| **Mandatory run variables**            | "sources": PPTX file paths or [`ByteStream`](/docs/data-classes#bytestresm)  objects           |
...
The `PPTXToDocument` component converts PPTX files into documents. It takes a list of file paths or [`ByteStream`](/docs/data-classes#bytestresm) objects as input and outputs the converted result as a list of documents. Optionally, you can attach metadata to the documents through the `meta` input parameter.
