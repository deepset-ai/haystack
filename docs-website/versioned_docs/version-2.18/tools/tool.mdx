...
If you are looking for the details of this data class's methods and parameters, visit our [API documentation](/reference/data-classes-api#module-tool).
...
The `Tool` class is used in Chat Generators and provides a consistent experience across models. `Tool` is also used in the [`ToolInvoker`](/docs/toolinvoker) component that executes calls prepared by Language Models.
...
See more details and examples on the [Toolset documentation page](/docs/toolset).

## Usage

To better understand this section, make sure you are also familiar with Haystack's [`ChatMessage`](/docs/chatmessage) data class.
...
To execute prepared tool calls, you can use the [`ToolInvoker`](/docs/toolinvoker) component. This component acts as the execution engine for tools, processing the calls prepared by the Language Model.
...
You can also use `Tool` with the [Agent](/docs/agent) component. Internally, the `Agent` component includes a `ToolInvoker` and the ChatGenerator of your choice to execute tool calls and process tool results.
