...
`GitHubIssueViewerTool` wraps the [`GitHubIssueViewer`](doc:githubissueviewer) component, providing a tool interface for use in agent workflows and tool-based pipelines.
...
You can use `GitHubIssueViewerTool` with the [Agent](/docs/agent) component. The Agent will automatically invoke the tool when needed to fetch GitHub issue information.
