...
The `ImageContent` dataclass represents an image object in Haystack pipelines. It encapsulates the **image data**, **metadata**, and a **unique identifier**, making it easy for pipeline components to process images consistently and modularly. It works seamlessly with other Haystack data classes like `ByteStream` and `Document` and can be used directly in pipelines for image processing.

**Key Features**
- Encapsulates image binary data and metadata.
- Provides utility methods for serialization and deserialization.
- Immutable — create new instances for modifications.
- Can be used in Haystack pipelines with nodes like `ImageToTextNode`.
...
| `metadata` | `Dict[str, Any]` | Optional metadata such as format, size, tags, or description. |
...
# Returns the image format from metadata
img.get_format()  

# Converts the ImageContent instance to a dictionary
img.to_dict()  

# Creates a new ImageContent instance from a dictionary
restored_image = ImageContent.from_dict(img.to_dict())  
```
...
from haystack.nodes import ImageToTextNode

# Create an ImageContent instance
image = ImageContent(
    image_id="img_001",
    data=open("cat.jpg", "rb").read(),
    metadata={"format": "JPEG", "description": "Cute cat image"}
)
...
print(restored_image.image_id)

# Run in a pipeline
pipeline = Pipeline()
pipeline.add_node(ImageToTextNode(), name="image_to_text")
result = pipeline.run(images=[image], query="What's in this image?")
```
