...
```python
__init__(json_schema: dict[str, Any] | None = None, error_template: str | None = None)
```
...
```python
run(messages: list[ChatMessage], json_schema: dict[str, Any] | None = None, error_template: str | None = None) -> dict[str, list[ChatMessage]]
```
