...
Performance may degrade for high-cardinality metadata fields and large from\_ values.
...
- **search_term** (<code>str | None</code>) – Optional case-insensitive substring to filter the returned values by, matched
  against the metadata field's own value (not the document content). NOTE: The matching is done
