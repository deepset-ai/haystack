...
To use the model-based Evaluators with a local model, pass a Chat Generator pointed at your local endpoint through the `chat_generator` parameter when initializing the Evaluator. The Chat Generator must be configured to return a JSON object.
