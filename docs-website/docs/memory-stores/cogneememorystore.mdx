...
- `self_improvement` is *optional* and defaults to `True`. When `True`, Cognee runs graph improvement after every write: inline on the permanent knowledge graph, and as a fire-and-forget background task on the session cache. Set to `False` when you want `improve()` to be the sole improvement trigger.
