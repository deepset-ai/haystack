...
add_tool = Tool(name="addition_tool", description="This tool adds two numbers", parameters=parameters, function=add)
...
image_retriever_tool = create_tool_from_function(function=retrieve_image, outputs_to_string={"raw_result": True})
...
def dummy_weather(location: str):
    return {"temp": f"{random.randint(-10, 40)} °C", "humidity": f"{random.randint(0, 100)}%"}
