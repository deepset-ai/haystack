...
:::tip
The recommended way to use tools in Haystack is through the [`Agent`](../pipeline-components/agents-1/agent.mdx) component, which manages the full tool call loop automatically. If you need fine-grained control over the loop, you can also drive tool calls manually: pass the tools to a Chat Generator, execute the requested tool calls with `Tool.invoke`, and send the results back as `ChatMessage.from_tool` messages.
:::
