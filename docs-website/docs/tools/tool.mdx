...
`create_tool_from_function` is the functional equivalent of `@tool` — useful when you're working with a function you can't decorate directly (e.g. a lambda, a method from a library, or a dynamically constructed callable). It accepts the same optional parameters as `@tool` and generates the JSON schema in the same way.
