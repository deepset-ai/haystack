...
### Manual Tool Calling with a Chat Generator

:::note
The following sections show the lower-level approach of driving tool calls yourself with a `ChatGenerator` and `Tool.invoke`. This is useful when you need precise control over the loop — for example, to add custom logic between steps — but for most use cases the `Agent` component above is simpler.
:::
...
The Language Model only prepares the tool call — it does not execute it. When driving the loop yourself, you execute the prepared calls with `Tool.invoke` and wrap each result in a tool `ChatMessage` using `ChatMessage.from_tool`. (When you use the `Agent` component, it does this for you.)
...
# If the assistant message contains tool calls, execute them and build tool messages
tool_messages = []
for tool_call in replies[0].tool_calls:
    result = weather.invoke(**tool_call.arguments)
...
Building on the [previous example](#executing-tool-calls), we send all messages back to the Chat Generator:

```python
# ... same setup as above (weather tool, chat_generator)
