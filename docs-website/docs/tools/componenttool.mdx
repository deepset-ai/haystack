...
You can also use `ComponentTool` in a pipeline by passing it to an `Agent` component.
...
pipeline.add_component(
    "agent",
    Agent(chat_generator=OpenAIChatGenerator(model="gpt-5.4-nano"), tools=[tool]),
)
