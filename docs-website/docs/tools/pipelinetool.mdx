...
`input_mapping` and `output_mapping`. It can be used with the `Agent` component, either directly or within a pipeline.
...
:::tip
The recommended way to use `PipelineTool` in Haystack is with the [`Agent`](../pipeline-components/agents-1/agent.mdx) component, which manages the tool call loop for you. You can run the `Agent` standalone or add it to a pipeline, as the examples below show.
:::
...
The examples on this page use Sentence Transformers components (a ranker and embedders) that have moved to the `sentence-transformers-haystack` package. Install it to run the examples:
...
You can also use `PipelineTool` in a pipeline by passing it to an `Agent` component.
