...
You can safely reuse the same `SearchableToolset` instance across multiple agent runs, including concurrent ones. The `Agent` internally gives each run an isolated copy of the toolset, so tools discovered in one run do not persist into, or collide with, other runs — every run starts fresh from the catalog:
