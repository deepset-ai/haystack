...
Alternatively, run your Haystack application using OpenTelemetry’s [automated instrumentation](https://opentelemetry.io/docs/languages/python/getting-started/#instrumentation). Haystack will automatically detect the configured tracing backend and use it to send traces.
...
1. Run the Jaeger container. This creates a tracing backend as well as a UI to visualize the traces:
   ...
   ```
2. Install the OpenTelemetry SDK and the integration:
