...
Instrumented applications typically send traces to a trace collector or a tracing backend. Haystack supports tracing through integrations for backends such as [OpenTelemetry](https://opentelemetry.io/), [Datadog](https://www.datadoghq.com/), [Langfuse](https://langfuse.com/), and [MLflow](https://mlflow.org/). You can also quickly implement support for additional providers of your choosing.
...
Haystack does not enable any tracing backend on its own. Tracing is activated explicitly through a tracing integration, for example by adding a connector such as the `OpenTelemetryConnector` to your pipeline.

To disable a tracer that you have enabled, call `disable_tracing` in Python:
...
3. Configure `OpenTelemetry` to use the Jaeger backend:
   ...
   ```
4. Install the `opentelemetry-haystack` integration and add the `OpenTelemetryConnector` to your pipeline to enable tracing:
   ...
   pipeline.add_component("tracer", OpenTelemetryConnector())
   ```
5. Run your pipeline:
