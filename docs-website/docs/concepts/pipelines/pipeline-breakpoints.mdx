...
Breakpoints inside an Agent, such as on its chat generator or on individual tools, are not supported in Haystack 3.0. To inspect an Agent's behavior, use [tracing](https://docs.haystack.deepset.ai/docs/tracing) instead.
