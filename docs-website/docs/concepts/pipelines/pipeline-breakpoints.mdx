...
When `snapshot_callback` is provided, file-saving is skipped; the callback is responsible for handling the snapshot. The same parameter is available on `Agent.run()` for agent breakpoints.
