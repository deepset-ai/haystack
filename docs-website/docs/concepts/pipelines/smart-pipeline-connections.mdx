...
* The target input is typed as `list`, `list | None`, a union of list types (e.g. `list[int] | list[str]`), or `Any`.
...
**Note:** If the target input is typed as `Any`, all connected outputs must still be list types.
