...
You control **when loops stop** either by designing your graph and routing logic carefully or by using built-in safety limits.

## How Loop Execution Works

Conceptually, a pipeline run works like this:

- **Graph of components**: A pipeline is a directed multigraph of components and their input/output sockets.
- **Inputs state**: The pipeline maintains an internal state of inputs for each component.
- **Ready components**: Components become runnable when all mandatory inputs are available.
- **Queue of work**: Runnable components are scheduled from a priority queue and executed one by one.
- **Outputs are fanned out**: A component’s outputs are written into the inputs of downstream components.

When your pipeline graph contains a cycle (for example, `validator` feeding back into `prompt_builder`), each new output that flows back into an upstream component can make that component runnable again.
This is how loops emerge naturally from the graph: the pipeline detects that the upstream component has new inputs available and runs it again.
...
For most loop scenarios, you don't need to think about these details – just connect components as usual and use `max_runs_per_component` to protect against mistakes.
...
2. **Relying on optional outputs**: When a component has multiple output sockets but only returns some of them, the unreturned outputs don't trigger their downstream connections. This can cause confusion in loops.
   ...
   **Recommended:** Use a `ConditionalRouter` with explicit, mutually exclusive conditions instead:
...
3. **User inputs retriggering the loop**: If a user-provided input is connected to a socket inside the loop, it might cause the loop to restart unexpectedly. See [Greedy vs. Lazy Variadic Sockets](#greedy-vs-lazy-variadic-sockets-in-loops).

4. **Multiple paths feeding the same component**: If a component inside the loop receives inputs from multiple sources, ensure you understand when it becomes runnable again.
...
3. **Visualize the pipeline**: Use `pipe.draw()` or `pipe.show()` to see the graph structure and verify your connections are correct.
