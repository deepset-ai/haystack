...
- `str` can connect to `ChatMessage`
- `ChatMessage` can connect to `str`
- `T` can connect to `list[T]`
- `list[str | ChatMessage]` can connect to `str` or `ChatMessage`
...
Previously, pipelines often required output adapters to normalize outputs between components.
With these flexible connections built-in, many common cases no longer need explicit adapters.
