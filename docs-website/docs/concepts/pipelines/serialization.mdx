...
- **Secrets** (`haystack.utils.auth.Secret`) — serialized and deserialized automatically so that sensitive values are not stored in plain text.
- **ComponentDevice** (`haystack.utils.device.ComponentDevice`) — device configuration is detected and restored automatically.
...
To serialize or deserialize a single component (for example, outside of a pipeline), you can use `component_to_dict` and `component_from_dict` from `haystack.core.serialization`. They use the default behavior above as a fallback when the component does not define custom `to_dict`/`from_dict`:
