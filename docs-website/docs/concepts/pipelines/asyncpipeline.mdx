...
The `AsyncPipeline` offers four ways to run your pipeline:
...
If a `streaming_callback` is set on a component (at init or at runtime through `data`), it is still invoked for each chunk in addition to the chunks being pushed to the handle.
