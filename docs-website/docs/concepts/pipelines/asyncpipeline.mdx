...
When streaming, components accept a sync `streaming_callback` in `run_async` too — see [Sync and Async Callbacks](../../pipeline-components/generators/guides-to-generators/choosing-the-right-generator.mdx#sync-and-async-callbacks) for details.
