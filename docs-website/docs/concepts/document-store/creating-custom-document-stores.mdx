...
The [tests](https://github.com/deepset-ai/haystack/blob/main/haystack/testing/document_store.py) in `DocumentStoreBaseTests`give you a good idea of
the overall expected behavior of a Document Store and the operations it should support, following it is a good way to make sure your implementation
is consistent with the rest of the Haystack ecosystem.

If the technology you are using for your Document Store supports asynchronous operations, we recommend implementing `async` versions of the methods
in the `DocumentStore` protocol as well. This will allow users to take advantage of async features in their applications and pipelines,
