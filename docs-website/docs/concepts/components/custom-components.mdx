...
When building custom components, do not mutate `run()` inputs in place (for example, editing a `dict`, `list`, or dataclass instance directly). Input values may be reused by other components or by later pipeline steps.

If you need to modify an input value, create a copy first and work on that copy:
