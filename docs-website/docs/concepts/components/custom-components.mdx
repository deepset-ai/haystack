...
When you need to modify nested mutable structures (e.g. `list` or `dict` attributes) or update many fields of the dataclass instance, use a full deep copy instead:
...
When your inputs may contain pipeline-specific types (`Component`, `Tool`, or `Toolset`) or other non-copyable objects, use Haystack's [`_deepcopy_with_exceptions`](https://github.com/deepset-ai/haystack/blob/main/haystack/core/pipeline/utils.py) from `haystack.core.pipeline.utils` instead of `copy.deepcopy`. It recursively copies containers but leaves those types by reference and returns the original object on copy failure (logging instead of raising), so the pipeline can safely pass data that includes tools or component references.
:::
