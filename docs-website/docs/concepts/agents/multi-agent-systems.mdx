...
Since `Agent` is a Haystack component, you can also wrap it with [`ComponentTool`](../../tools/componenttool.mdx). However, this exposes the agent's full component interface to the coordinator, including arguments such as `tools`, `generation_kwargs`, and `hook_context`, and the coordinator receives the complete result dictionary. You can reduce the output with `outputs_to_string={"source": "last_message"}`, but the returned message still includes its serialized metadata. With `AgentTool`, the agent's configuration and execution details stay hidden behind the tool boundary.
...
The example below asks the coordinator about two topics: it calls `research_specialist` twice in a single response, and both specialists run in parallel.
