...
`Document` class contains information to be carried through the pipeline. It can be text, metadata, binary data, or vector representations. Documents can be written into Document Stores but also written and read by other components.
...
If you want to reuse pipelines, you can save them to disk in YAML format or share them around using the [serialization](pipelines/serialization.mdx) process.
...
A pipeline runs the same graph every time - predictable, but fixed. An [Agent](agents.mdx) hands step-by-step control to the LLM instead: it decides which tool to call next based on what the previous step returned, which suits open-ended tasks where the right sequence of steps isn't known upfront.

An `Agent` is itself a component, so you can use one inside a pipeline, or wrap a whole pipeline as a tool for an agent to call. See [Choosing Between Pipelines and Agents](agents/choosing-between-pipelines-and-agents.mdx) for how to decide, and [Multi-Agent Systems](agents/multi-agent-systems.mdx) for composing several agents together.
