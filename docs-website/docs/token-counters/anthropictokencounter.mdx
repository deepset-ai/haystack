...
You can count tool schemas without messages by calling `counter.count([], tools=[search_tool])`. When both `messages` and `tools` are empty, `count()` returns `0` without calling the API.
