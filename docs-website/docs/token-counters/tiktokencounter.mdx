...
Strings that look like special tokens, such as `<|endoftext|>`, are counted as ordinary text. They are measured like any other source text, so a message or tool schema that contains one is not rejected.
