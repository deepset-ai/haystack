...
This example uses the `MetaFieldRanker` to rank two simple documents. `MetaFieldRanker` ranks by meta field value only, so `run()` takes no `query`: you provide the `documents` and set the number of documents to return using the `top_k` parameter.
