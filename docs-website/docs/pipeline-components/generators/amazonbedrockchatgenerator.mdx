...
Each request can contain up to 4 cache points.

#### Caching messages

This generator allows you control cache points at the `ChatMessage` level via the `meta` field.

For example, to cache a long user message that will be reused across multiple requests:
```python
