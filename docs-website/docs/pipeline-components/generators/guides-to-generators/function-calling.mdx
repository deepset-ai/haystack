...
description: "Learn how function calling works in Haystack and what happens under the hood when an Agent runs tools."
---

# Function Calling

Learn how function calling works in Haystack and what happens under the hood when an [`Agent`](../../agents-1/agent.mdx) runs tools.
...
In practice, you rarely drive this loop yourself: the `Agent` component manages it for you. This page walks through the individual steps of a tool call so you can understand what the `Agent` automates — useful for debugging agents or building a custom loop when you need fine-grained control.
...
The model doesn't actually call the function. Function calling returns the name of a function and the arguments to invoke it. The actual invocation is performed by your code (or by a Haystack component such as `Agent`).
:::

## Example

Let's walk through the three steps the `Agent` performs on every tool call: define a tool, let the LLM pick it, and then actually invoke it.

:::tip[Real-world usage]

We split function calling into separate steps below to show what happens under the hood. In real applications, the [`Agent`](../../agents-1/agent.mdx) component handles the full loop for you. See [Using the Agent component](#using-the-agent-component) at the end of this section.
:::
...
To execute the requested tool calls, call `Tool.invoke` with the arguments the model prepared, then wrap each result in a [`ChatMessage`](../../../concepts/data-classes/chatmessage.mdx) with role `tool` using `ChatMessage.from_tool`. To get a final natural-language answer, feed those results back to the ChatGenerator.

```python
# Run the tools requested by the assistant.
tool_messages = []
for tool_call in response["replies"][0].tool_calls:
    result = get_weather.invoke(**tool_call.arguments)
    tool_messages.append(
        ChatMessage.from_tool(tool_result=str(result), origin=tool_call),
    )
...
In real applications, you typically don't manage the loop yourself: the [`Agent`](../../agents-1/agent.mdx) component does it for you. Internally, it wraps a ChatGenerator with built-in tool execution and iterates until the model produces a final answer.
