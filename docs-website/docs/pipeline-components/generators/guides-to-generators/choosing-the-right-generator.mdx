...
If you need custom rendering (for example, SSE or WebSocket transport), write your own callback. Handle the four chunk types in order:
...
def my_streaming_callback(chunk: StreamingChunk) -> None:
    # Tool calls: name and JSON arguments arrive as streaming deltas
    if chunk.tool_calls:
        for tool_call in chunk.tool_calls:
            ...
            if tool_call.arguments:
                print(tool_call.arguments, end="", flush=True)

    # Tool result: fully formed output from the tool, emitted by ToolInvoker
    if chunk.tool_call_result:
        print(f"\n[TOOL RESULT]\n{chunk.tool_call_result.result}", end="", flush=True)

    # Text tokens: normal assistant response, arrives word by word
    if chunk.content:
        if chunk.start:
            print("\n[ASSISTANT]\n", end="", flush=True)
        print(chunk.content, end="", flush=True)

    # Reasoning tokens: extended thinking output (for models that support it)
    if chunk.reasoning:
        if chunk.start:
            print("\n[REASONING]\n", end="", flush=True)
        print(chunk.reasoning.reasoning_text, end="", flush=True)
