...
In Haystack, ChatGenerators are the main interface for interacting with Generative Language Models.
...
In earlier versions, Haystack shipped two flavors of Generators: plain Generators (such as `OpenAIGenerator`), which took a prompt string, and ChatGenerators (such as `OpenAIChatGenerator`), which work with the [ChatMessage data class](../../../concepts/data-classes/chatmessage.mdx).

- **ChatGenerators** accept either a plain prompt string or a list of Chat Messages, and return Chat Messages in “replies”. They support Function Calling and Multimodal inputs.
- Haystack core now ships only ChatGenerators, plus the special-purpose `OpenAIImageGenerator` for image generation. Some integrations still provide non-chat Generators.

:::warning
The legacy non-chat Generators (`OpenAIGenerator`, `AzureOpenAIGenerator`, `HuggingFaceAPIGenerator`, `HuggingFaceLocalGenerator`) were removed in Haystack 3.0. Use the corresponding ChatGenerators instead — they also accept a plain string prompt, so simple text-in/text-out use cases keep working.
:::
