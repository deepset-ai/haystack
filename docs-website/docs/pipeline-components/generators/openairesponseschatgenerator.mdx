...
`OpenAIResponsesChatGenerator` supports structured output generation through the `text_format` parameter in `generation_kwargs`. You can provide a Pydantic model to define the structure of the output:
...
- Only Pydantic models are supported right now. Passing raw JSON schemas is not yet supported by this component.
- Structured outputs require the latest models starting from GPT-4o.
- Older models only support the basic JSON mode through `{"type": "json_object"}`. For details, see [OpenAI JSON mode documentation](https://platform.openai.com/docs/guides/structured-outputs#json-mode).
