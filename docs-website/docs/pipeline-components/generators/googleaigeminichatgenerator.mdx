...
`GoogleAIGeminiChatGenerator` supports Gemini models such as `gemini-3.8-flash` (the default), `gemini-3.7-flash`, `gemini-2.5-pro`, and `gemini-2.5-flash`. Deprecated models like `gemini-2.0-flash` and `gemini-1.5-pro` are no longer available.
...
gemini_chat = GoogleAIGeminiChatGenerator(model="gemini-3.8-flash", tools=[tool])
```
...
agent = Agent(
    chat_generator=GoogleAIGeminiChatGenerator(model="gemini-3.8-flash"),
    tools=[tool],
)
