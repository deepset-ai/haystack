...
`AzureOpenAIResponsesChatGenerator` supports structured output generation through the `text_format` parameter in `generation_kwargs`. Provide a Pydantic model to define the structure of the output:
