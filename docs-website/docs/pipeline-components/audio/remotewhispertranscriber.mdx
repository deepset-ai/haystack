...
`RemoteWhisperTranscriber` works with OpenAI-compatible clients and isn't limited to just OpenAI as a provider. For example, [Groq](https://console.groq.com/docs/speech-to-text) offers a drop-in replacement that can be used as well. You can set the API key in one of two ways:
