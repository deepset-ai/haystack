...
| --- | --- | --- |
| [`SlidingWindowCompactor`](compaction/sliding-window-compactor.mdx) | Preserves the Agent's instructions, latest user task, and recent complete Agent steps while removing older history. | Fast and local, but discarded information is not summarized. |
