...
To change anything else, such as adding tools, [hooks](../hooks.mdx), or [`State`](../state.mdx) entries, use [`clone()`](../agent.mdx#cloning-and-modifying-an-agent) on the returned agent. Unpack the existing values to keep the built-in tools and hooks:
