...
The hook offloads a result as soon as it is produced, before the model has seen it. To keep fresh results directly in context and start offloading only once the conversation grows too large, use [`ToolResultOffloadCompactor`](./compaction/tool-result-offload-compactor.mdx) instead: it writes to the same stores in the same pointer format, but runs from `CompactionHook` rather than after every tool call.
...
- [ToolResultOffloadCompactor](./compaction/tool-result-offload-compactor.mdx) — the same offloading, deferred until context compaction is triggered
