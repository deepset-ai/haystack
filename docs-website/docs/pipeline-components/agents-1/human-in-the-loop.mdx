...
agent = Agent(
    chat_generator=OpenAIChatGenerator(model="gpt-5.4-mini"),
    tools=[send_email_tool, delete_record_tool, search_tool],
    confirmation_strategies={
        # Ask every time for send_email
        "send_email": BlockingConfirmationStrategy(
            confirmation_policy=AlwaysAskPolicy(),
            confirmation_ui=SimpleConsoleUI(),
        ),
        # Share one strategy across multiple sensitive tools
        ("delete_record", "update_record"): BlockingConfirmationStrategy(
            confirmation_policy=AlwaysAskPolicy(),
            confirmation_ui=SimpleConsoleUI(),
        ),
        # search_tool has no strategy — always executes without asking
    },
)
```
