...
`SlidingWindowCompactor` removes older conversation history while preserving the Agent's instructions, current task, and as much complete recent conversation as the token target allows. It removes earlier user/assistant turns as whole units first, and only trims the current task's own Agent steps when removing every earlier turn is not enough.
...
The compactor divides a conversation into protected context, earlier turns, and the current task's Agent steps:
...
3. It groups the history before that task into complete user turns, each running from one user message up to the next.
4. It groups each assistant message and all immediately following tool-result messages into one complete Agent step.
5. Working backwards from the newest, it keeps as many whole earlier turns as fit within the target.
