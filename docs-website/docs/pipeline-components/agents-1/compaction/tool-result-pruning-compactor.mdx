...
2. It always protects the complete trailing batch of results from the current Agent step because the model has not acted on them yet. If `min_keep_results` is higher than the number of results in that batch, it protects enough additional recent results to reach that minimum.
3. It skips errors, results already marked by context compaction, results below the token threshold, and results carrying protected metadata such as an offloaded-result reference.
