...
2. It protects results from at least the configured number of recent tool-calling Agent steps. The complete trailing batch from the current step is always protected because the model has not acted on it yet.
...
| --- | --- | --- |
| `min_keep_steps` | `1` | The minimum number of recent tool-calling Agent steps whose results remain intact regardless of the target. Parallel results from one step are protected together. The complete current-step result batch is always protected. |
...
- There are no results older than the protected steps or current-step batch.
