...
| Compactor | Strategy |
| --- | --- |
| [`SlidingWindowCompactor`](sliding-window-compactor.mdx) | Keeps the current task and as much complete recent conversation as fits, dropping earlier turns whole before it trims the task's own steps. |
