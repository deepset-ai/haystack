...
When building agents that use multiple tools, you often need tools to share information.
State provides centralized storage that all tools can read from and write to.
For example, one tool might retrieve documents while another uses those documents to generate an answer.
...
Custom handlers are useful when the default `merge_lists` or `replace_values` behaviors don't fit your needs. A handler takes the current state value and the new value and returns the merged result.
...
Parameters mapped via `inputs_from_state` are automatically excluded from the LLM's parameter schema — the model never sees or provides them:
...
A research agent that accumulates retrieved documents in state alongside the final answer.
Because `list[Document]` uses `merge_lists` by default, documents from every search call are appended — so if the agent searches multiple times, all results are available for inspection after the run.
...
@tool(outputs_to_state={"documents": {"source": "documents"}})
def search(query: Annotated[str, "The search query"]) -> dict:
    """Search for documents about a topic and return relevant results."""
    # In a real application, replace this with a call to a retriever or search API
    return {
        "documents": [
            Document(
                content=f"Result 1 for '{query}'",
                meta={"source": "https://example.com/1"},
            ),
            Document(
                content=f"Result 2 for '{query}'",
                meta={"source": "https://example.com/2"},
            ),
        ],
    }
...
agent = Agent(
    chat_generator=OpenAIChatGenerator(model="gpt-5.4-nano"),
    tools=[search],
    system_prompt="Use the search tool to find information. Search multiple times if needed to answer thoroughly.",
    streaming_callback=print_streaming_chunk,
    state_schema={"documents": {"type": list[Document]}},
)

result = agent.run(
    messages=[ChatMessage.from_user("What are the main features of Haystack?")],
)

print(result["last_message"].text)

print(f"\nSources ({len(result['documents'])} documents retrieved):")
for doc in result["documents"]:
    print(f"  [{doc.meta['source']}] {doc.content}")
```

Some more advanced patterns you could build on top of this:

- **Retrieve and filter pipeline**: add a second tool that reads `documents` from state via `inputs_from_state`, filters or re-ranks them, and writes the filtered subset back. See [Combining Inputs and Outputs](#combining-inputs-and-outputs).
- **Direct state inspection inside a tool**: replace `outputs_to_state` with a `state: State` parameter to read and write multiple keys in one tool call. See [Injecting State Directly into Tools](#injecting-state-directly-into-tools).
