...
Because `list[Document]` uses `merge_lists` by default, documents from every search call are appended.
If the agent searches multiple times, all results are available for inspection after the run.
