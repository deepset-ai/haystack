...
- `state_schema`: Defines the agent's runtime state — a dict mapping key names to type configs (e.g. `{“docs”: {“type”: list[Document]}}`). Tools can read from and write to state keys via `inputs_from_state` and `outputs_to_state`.
...
When wrapping an `Agent` as a `ComponentTool`, use `outputs_to_string={"source": "last_message"}` to pass only the specialist's final reply to the coordinator, keeping its context clean rather than forwarding the full tool call trace.
...
fetch_tool = ComponentTool(
    component=LinkContentFetcher(),
    name="fetch_page",
    description="Fetch the full content of a web page given its URL",
)
