...
| **Output variables**                   | `messages`: Chat history with tool and model responses  <br /> <br />`last_message`: The final `ChatMessage` of the run  <br /> <br />`step_count`, `token_usage`, `tool_call_counts`, `exit_reason`: Run metadata  <br /> <br />Plus one output per key defined in `state_schema` |
...
- `"client_tools"`: the model called a tool from `generation_kwargs["tools"]` that the Agent does not invoke (typical for OpenAI-compatible proxies that forward a client-side tool catalog). `last_message` is the assistant message with the pending tool call when only client tools were requested; when the model also called Agent tools in the same step, `last_message` is the last tool result and the pending client call remains in `messages`.
- `"max_agent_steps"`: the agent reached `max_agent_steps` before meeting an exit condition.
- a custom reason set by a hook through the `stop_run` state key, such as `"token_budget_exceeded"` from the [`TokenBudgetHook`](./token-budget.mdx).
...
- `generation_kwargs`: Additional keyword arguments forwarded to the chat generator (e.g. `{"temperature": 0.2}`). They are merged per key with the `generation_kwargs` set at the chat generator's initialization: keys passed here take precedence, keys set only at initialization are kept. A `tools` entry is treated as extra **client-side** OpenAI function specs for the LLM: the Agent offers them alongside its own tools, strips them from the payload forwarded as `generation_kwargs`, and never invokes them. Tool names the Agent does not own and that are not listed in `generation_kwargs["tools"]` still follow the normal tool-not-found path.
...
Two things are required: set `outputs_to_string={"raw_result": True}` so the Agent's tool execution skips string conversion, and return a `list[ImageContent]` (the tool result type is `str | Sequence[TextContent | ImageContent]`).
...
The simplest way is [`AgentTool`](../../tools/agenttool.mdx), which wraps an `Agent` and delegates a task to it as a single user message, returning only its final reply.

See [Multi-Agent Systems](../../concepts/agents/multi-agent-systems.mdx) for a full guide.
