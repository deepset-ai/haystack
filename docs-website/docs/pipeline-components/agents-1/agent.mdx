...
When wrapping an `Agent` as a `ComponentTool`, use the `outputs_to_string` parameter with `{"source": "last_message"}` to extract only the agent's final response text, rather than the entire conversation history with tool calls to keep the coordinator agent's context clean and focused.
...
## Define a tool for the specialist agent
def search_knowledge_base(query: str) -> dict:
    """Search internal knowledge base for information."""
    ## In a real application, this would query a document store
    knowledge = {
        "Haystack": "Haystack is an open-source framework for building AI applications."
    }
    return {"result": knowledge.get(query, f"No information found for: {query}")}
...
research_agent = Agent(
    ...
    tools=[search_tool],
    system_prompt="You are a research specialist. Use the search tool to find information and provide concise answers.",
