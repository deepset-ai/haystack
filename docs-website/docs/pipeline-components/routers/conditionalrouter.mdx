...
By default, `ConditionalRouter` does not check that a route's output value actually matches the declared `output_type`.
Set `validate_output_type=True` to enable this check.
If the value doesn't match, a `ValueError` is raised at runtime.
