...
The `google-vertex-haystack` integration is archived and no longer maintained. It builds on a deprecated Google SDK.

We recommend switching to the [GoogleGenAIDocumentEmbedder](googlegenaidocumentembedder.mdx) from the `google-genai-haystack` package instead.
:::
