...
You can pass vLLM-specific parameters through the `extra_parameters` dictionary. These are forwarded as `extra_body` to the OpenAI-compatible embeddings endpoint. Use this to pass parameters that are not part of the standard OpenAI Embeddings API, such as `truncate_prompt_tokens`, `truncation_side`, `additional_data`, or `use_activation`. See the [vLLM Embeddings API docs](https://docs.vllm.ai/en/stable/models/pooling_models/embed/#openai-compatible-embeddings-api) for details.
...
Some embedding models require prepending the text with an instruction to work better for retrieval. For example, if you use [google/embeddinggemma-300m](https://huggingface.co/google/embeddinggemma-300m), you should prefix your query with the following instruction: "task: search result | query: ".
...
Pass vLLM-specific parameters through the `extra_parameters` dictionary:
