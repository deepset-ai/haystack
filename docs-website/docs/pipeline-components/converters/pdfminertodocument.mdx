...
By default, `PDFMinerToDocument` extracts only the visible text of hyperlinks. To include hyperlink targets in the document content, set `link_format` to `"markdown"` for `[text](url)` output or `"plain"` for `text (url)` output.
