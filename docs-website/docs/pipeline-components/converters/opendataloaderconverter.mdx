...
Image extraction is turned off by default so that documents contain text only. Turn it back on through `convert_kwargs` if you need images.
...
Pass any [OpenDataLoader PDF option](https://opendataloader.org/docs/quick-start-python) through `convert_kwargs`. For example, to convert a page range of an encrypted PDF, keep page separators in the Markdown output, and redact sensitive data:
