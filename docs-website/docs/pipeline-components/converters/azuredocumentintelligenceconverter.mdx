...
:::info
This component replaces the legacy [`AzureOCRDocumentConverter`](/azureocrdocumentconverter), which uses the older `azure-ai-formrecognizer` package. The `AzureDocumentIntelligenceConverter` uses the newer `azure-ai-documentintelligence` SDK and produces Markdown output instead of plain text, making it better suited for LLM and RAG applications.
:::
