...
In row mode (`conversion_mode="row"`), pass `content_column` to select the column used as document content.
The remaining columns become metadata, while `meta["row_number"]` records the zero-based row index.
A CSV column named `row_number` is preserved as `csv_row_number`; if that key already exists in the metadata,
a numeric suffix is added, for example `csv_row_number_1`.
