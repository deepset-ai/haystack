...
`FileToFileContent` processes a list of file sources and converts them into `FileContent` objects.
...
Support for this feature differs by LLM provider. Some providers do not support it, some restrict it to PDF files, and others allow a wider range of file types.
...
results = pipeline.run({
    "fetcher": {
        "urls": ["https://arxiv.org/pdf/2309.08632"]
    }
})
