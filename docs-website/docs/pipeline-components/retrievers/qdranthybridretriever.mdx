...
For an existing collection, follow the [sparse migration guidance](../../document-stores/qdrant-document-store.mdx#migrating-to-sparse-embeddings), including the supported deployment modes and the separate step to compute sparse embeddings.
:::
