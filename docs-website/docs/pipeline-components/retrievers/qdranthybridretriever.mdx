...
When using the `QdrantHybridRetriever`, make sure it has query and documents with dense and sparse embeddings available. You can do so by:
...
If you want to use a Document Store or a collection previously created with this feature disabled, you must migrate the existing data. You can do this by taking advantage of the `migrate_to_sparse_embeddings_support` utility function.
:::
...
indexing.add_component(
    "sparse_doc_embedder",
    FastembedSparseDocumentEmbedder(model="prithivida/Splade_PP_en_v1"),
)
indexing.add_component(
    "dense_doc_embedder",
