...
The Java document is in the list because the default `fuzzy` mode is deliberately permissive: it combines the fuzzy match, a wildcard, and a wildcard `query_string` clause in a `dis_max` query with no minimum-match requirement, so any document that has the searched fields can become a candidate. Relevance is what separates them, and the Java entry is ranked last. Use `mode="strict"` when you only want documents that actually match the query, or lower `top_k`.
