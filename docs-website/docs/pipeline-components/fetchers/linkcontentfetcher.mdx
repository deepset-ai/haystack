...
`LinkContentFetcher` will request the URLs you pass to it. If those URLs come directly from end users, this can expose your environment to server-side request forgery (SSRF) risks.

Before calling `LinkContentFetcher`, validate and sanitize user-provided URLs:
...
def is_safe_url(url: str) -> bool:
    ...
    try:
        ...
        # Hostname (not a raw IP). Apply your own domain allowlist policy here.
        return True
    return not any(host_ip in net for net in PRIVATE_RANGES)
```
