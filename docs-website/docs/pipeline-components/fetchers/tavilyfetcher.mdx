...
`TavilyFetcher` also supports asynchronous execution through `run_async()`. The underlying clients are created lazily on the first call. To avoid the cold-start latency of the first call, you can call `warm_up()` explicitly.
