...
Firecrawl returns content in a structured format (Markdown by default) that works well as input for LLMs. Each crawled page becomes a separate `Document` with the page content in the `content` field and metadata (title, URL, description, and so on) in the `meta` field.
...
for doc in documents:
    print(doc.meta.get("title"), "-", doc.meta.get("sourceURL"))
```
