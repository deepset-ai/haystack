...
| **Mandatory init variables**           | None. `top_p`, the cumulative probability threshold for document selection, defaults to `1.0` (no filtering) |
...
The component’s `run()` method takes in a set of documents that already carry scores and filters them based on the cumulative probability of those scores. It doesn't compute scores itself, so place it after a component that does, such as a Ranker.
...
The examples on this page use Sentence Transformers rankers and the SerperDev web search component from the `sentence-transformers-haystack` and `serperdev-haystack` packages. Install them to run the examples:
