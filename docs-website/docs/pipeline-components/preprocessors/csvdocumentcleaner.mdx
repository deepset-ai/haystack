...
- `keep_embedding`: Preserves the document's vector embeddings (`embedding` and `sparse_embedding`). The default value is `false`, which clears embeddings to `null` because the CSV content is re-serialized during cleaning.
