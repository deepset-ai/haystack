...
- `keep_embedding` preserves the document's vector embeddings (`embedding` and `sparse_embedding`). The default value is `false`, which clears embeddings to `null` because document content changes during cleaning.
