...
Because it loads an embedding model, this component requires `warm_up()` to be called before use.
In a Haystack pipeline, `warm_up()` is called automatically when the pipeline is run.
When using the component standalone, call `warm_up()` explicitly.
...
In a Haystack pipeline, `warm_up()` is called automatically before the pipeline runs.
