...
Split documents at ATX-style Markdown headers (`#`, `##`, etc.), with optional secondary splitting. Header hierarchy is preserved as metadata on each chunk.
...
| **Mandatory init variables**           | None; all parameters have defaults. |
...
Only ATX-style headers are recognized (e.g. `# Title`); Setext-style headers (`Underline with ===`) are not supported.
...
The component only works with text documents (string content). Documents with `None` or non-string content will raise a `ValueError`.
...
# with meta["header"], meta["parent_headers"], meta["source_id"], etc.
```

### With secondary splitting

When sections are long, you can add a secondary split (e.g. by word) so each chunk stays within a maximum size:
