...
Search the web using the Linkup Search API, optimized for LLM and agent applications.
...
When you give `LinkupWebSearch` a query, it uses the [Linkup](https://www.linkup.so) Search API to search the web and return the relevant content as Haystack `Document` objects. It also returns a list of the source URLs.

Linkup is a search API built for LLM and agent applications. It returns clean, ready-to-use content along with the sources, which makes it a good fit for RAG pipelines.

Use the `depth` parameter to trade latency for thoroughness:

- `"fast"`: sub-second results for keyword-based queries (beta).
- `"standard"`: a simple search. This is the default.
- `"deep"`: a more powerful agentic workflow that takes longer but handles complex queries better.

`top_k` limits the number of results and maps to the `max_results` parameter of the Linkup API. To use additional API options, such as `include_images`, `from_date`, `to_date`, `include_domains`, or `exclude_domains`, pass them in `search_params`. See the [Linkup API reference](https://docs.linkup.so/pages/documentation/api-reference/endpoint/post-search) for all available options.
