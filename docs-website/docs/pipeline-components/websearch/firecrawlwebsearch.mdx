...
Because Firecrawl actively scrapes and structures the content of the pages it finds into LLM-friendly formats, you generally do not need an additional component like `LinkContentFetcher` to read the web pages. `FirecrawlWebSearch` handles the retrieval and scraping all in one step.
...
Here is a quick example of how `FirecrawlWebSearch` searcher the web based on a query, scrapes the resulting web pages, and returns a list of Documents containing the page content.
...
Here is an example of a Retrieval-Augmented Generation (RAG) pipeline where we use `FirecrawlWebSearch` to look up an answer. Because Firecrawl returns the actual text of the scraped pages, we can pass its `documents` output directly into the `ChatPromptBuilder` to give our LLM the necessary context.
