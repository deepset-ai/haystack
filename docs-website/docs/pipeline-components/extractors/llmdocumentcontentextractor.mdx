...
| **Mandatory init variables** | `chat_generator`: A ChatGenerator instance that supports vision-based input |
| **Optional init variables** | `prompt`: Instructional text for the LLM on how to extract content (no Jinja variables allowed). Defaults to a built-in extraction prompt. |
