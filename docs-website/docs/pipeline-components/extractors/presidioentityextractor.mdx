...
If you want to replace PII directly rather than annotate it, see [`PresidioDocumentCleaner`](/presidiodocumentcleaner) for Documents or [`PresidioTextCleaner`](/presidiotextcleaner) for plain strings.
...
# Documents are written with a "entities" key in their metadata
```
