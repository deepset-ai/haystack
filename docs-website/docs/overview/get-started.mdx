...
`HuggingFaceAPIChatGenerator` moved to its own package, so install it to run the example:
...
All the examples below use `SerperDevWebSearch`, which lives in the `serperdev-haystack` package:

```shell
pip install serperdev-haystack
```

They also require a [SerperDev API key](https://serper.dev/) for web search. Set it as the `SERPERDEV_API_KEY` environment variable.
