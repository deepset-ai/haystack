...
Both frameworks connect tools to the LLM, but with different APIs. In Haystack, you initialize the `ChatGenerator` component and pass the tools to the `Agent` (Step 6), which forwards them to the LLM. In LangGraph, you first initialize the model, then bind tools using `.bind_tools()` to create a tool-enabled LLM instance.
...
# Initialize the LLM; the tools are passed to the Agent in Step 6
...
This is where the frameworks diverge more significantly. In Haystack, the `Agent` component accumulates the conversation history (LLM replies and tool results) across the agentic loop internally, so there is nothing to set up. LangGraph instead defines a node function (`llm_call`) that operates on `MessagesState` - a built-in state container that automatically manages message history.

<div className="code-comparison">
  <div className="code-comparison__column">
    <CodeBlock language="python" title="Haystack">{`# No message-handling setup is needed in Haystack:
# the Agent component (created in Step 6) accumulates
# the conversation - LLM replies and tool results -
# internally during the agentic loop.`}</CodeBlock>
  </div>
  <div className="code-comparison__column">
...
When the LLM decides to use a tool, it must be invoked and its result returned. Haystack's `Agent` handles this automatically: it executes the tool calls prepared by the LLM and appends the results to the conversation. LangGraph requires you to define a custom node function that iterates over tool calls, invokes each tool, and wraps the results in `ToolMessage` objects.

<div className="code-comparison">
  <div className="code-comparison__column">
    <CodeBlock language="python" title="Haystack">{`# Tool execution is built into the Agent component:
# it runs the tool calls prepared by the LLM and adds
# the results to the conversation automatically.`}</CodeBlock>
  </div>
  <div className="code-comparison__column">
...
After the LLM responds, we need to decide whether to continue the loop (if tools were called) or finish (if the LLM provided a final answer). Haystack's `Agent` controls this through its `exit_conditions` parameter: the default `["text"]` stops the loop as soon as the LLM replies without tool calls, and you can also exit after specific tools run. LangGraph uses a conditional edge function (`should_continue`) that inspects the state and returns the next node or `END`.

<div className="code-comparison">
  <div className="code-comparison__column">
    <CodeBlock language="python" title="Haystack">{`# The Agent loops until an exit condition is met.
# The default exit_conditions=["text"] stops the loop
# when the LLM replies without tool calls. Tool names
# can also be used to exit after a specific tool runs.`}</CodeBlock>
  </div>
  <div className="code-comparison__column">
...
This is where you wire together the workflow. In Haystack, you create the `Agent` component from the chat generator and tools - the agentic loop (LLM calls, tool execution, iteration) is built in. LangGraph uses a `StateGraph` where you add nodes and edges, then compile the graph into an executable agent. Both approaches achieve the same agentic loop, but with different levels of explicitness.

<div className="code-comparison">
  <div className="code-comparison__column">
    <CodeBlock language="python" title="Haystack">{`from haystack.components.agents import Agent

# Create the agent - the agentic loop
# (LLM calls, tool execution, iteration) is built in
agent = Agent(
...
<div className="code-comparison">
  <div className="code-comparison__column">
    <CodeBlock language="python" title="Haystack">{`from haystack.dataclasses import ChatMessage

# Run the agent
result = agent.run(messages=[
    ChatMessage.from_user(text="Add 3 and 4.")
])
print(result["last_message"].text)`}</CodeBlock>
  </div>
  <div className="code-comparison__column">
