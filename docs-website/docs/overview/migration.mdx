...
id: migration
slug: "/migration"
description: "Learn how to make the move to Haystack 3.0 from Haystack 2.x."
---

# Migration Guide

Learn how to make the move to Haystack 3.0 from Haystack 2.x.

This guide is designed for those with previous experience with Haystack 2.x who want to upgrade to Haystack 3.0. It walks through every breaking change and shows how to adapt your code. If you're new to Haystack, skip this page and proceed directly to the [Get Started](get-started.mdx) guide.

Haystack 3.0 is an evolution of Haystack 2.x, not a rewrite: components, pipelines, and the `Agent` work as before. Most applications only need import updates and small, mechanical changes. The complete list of breaking changes with extended examples is maintained in [MIGRATION.md](https://github.com/deepset-ai/haystack/blob/main/MIGRATION.md) in the Haystack repository.
...
`OpenAIGenerator`, `AzureOpenAIGenerator`, `HuggingFaceAPIGenerator`, and `HuggingFaceLocalGenerator` have been removed. Use their chat counterparts instead — all [ChatGenerators](../pipeline-components/generators.mdx) now also accept a plain `str` as input, so simple text-in/text-out use cases rarely require structural changes.
