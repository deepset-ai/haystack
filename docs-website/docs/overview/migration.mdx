...
The [_Migration examples_](#migration-examples) section below shows how to port a `PromptNode` using OpenAI with a prompt template to a corresponding pipeline using the `OpenAIChatGenerator` in conjunction with a `PromptBuilder` component.
...
In current versions of Haystack, you build Agents with the standalone [`Agent`](../pipeline-components/agents-1/agent.mdx) component, which combines a Chat Generator with Tools and runs the tool-calling loop for you.
