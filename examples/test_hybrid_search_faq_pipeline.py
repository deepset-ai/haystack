from examples.hybrid_search_faq_pipeline import hybrid_search_faq_pipeline

from haystack.schema import Answer, Document


def test_basic_faq_pipeline():
    prediction = hybrid_search_faq_pipeline()

    assert prediction is not None
    assert prediction["query"] == "How is the virus spreading?"

    assert len(prediction["answers"]) == 5  # top-k of ReRanker
