...
To be able to make a statement about the performance of a question-answering pipeline or any other pipeline haystack, it is important to evaluate it. Furthermore, evaluation allows to determine which components of the pipeline can be improved.
...
# Alternative if Docker is available
...
## Evaluation of an ExtractiveQAPipeline, which consists of a Retriever and Reader (Open Domain)
