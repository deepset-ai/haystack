...
<a id="farm.FARMRanker.__init__"></a>

#### \_\_init\_\_

```python
def __init__(model_name_or_path: Union[str, Path], model_version: Optional[str] = None, batch_size: int = 50, use_gpu: bool = True, top_k: int = 10, num_processes: Optional[int] = None, max_seq_len: int = 256, progress_bar: bool = True)
```
...
<a id="farm.FARMRanker.update_parameters"></a>

#### update\_parameters

```python
def update_parameters(max_seq_len: Optional[int] = None)
```

Hot update parameters of a loaded Ranker. It may not to be safe when processing concurrent requests.

<a id="farm.FARMRanker.save"></a>
...
<a id="farm.FARMRanker.predict_batch"></a>
...
<a id="farm.FARMRanker.predict"></a>
