...
File converter that uses the open-source Parsr tool by axa-group.
(https://github.com/axa-group/Parsr).
This Converter extracts both text and tables.
It supports PDF and DOCX files. 
...
- `extractor`: Backend used to extract text structured from PDFs ("pdfminer" or "pdfjs").
...
- `preceding_context_len`: The number of lines that precede a table to extract as preceding context
(this number is returned as part of meta data).
- `following_context_len`: The number of lines following a table to extract as preceding context
(this number is returned as part of meta data).
- `remove_page_headers`: Removes the text that Parsr detected as a page header. Possible values: True, False.
- `remove_page_footers`: Removes the text that Parsr detected as a page footer. Possible values: True, False.
- `remove_table_of_contents`: Removes the text that Parsr detected as a table of contents. Possible values: True, False.
- `valid_languages`: Validate languages from a list of languages specified in the [ISO 639-1]
(https://en.wikipedia.org/wiki/ISO_639-1) format.
Use this option to add test for encoding errors. If the extracted text is
not one of the valid languages, it might likely be encoding error resulting
in garbled text.
- `id_hash_keys`: Generate the document ID from a custom list of strings that refer to the document's
attributes. To ensure you don't have duplicate documents in your DocumentStore but texts are
not unique, you can modify the metadata and pass for example, `"meta"` to this field ([`"content"`, `"meta"`]).
If you do this, the ID is generated by using the content and the defined metadata.
- `add_page_number`: Whether to add the number of the page a table occurs in to the Document's meta field
`"page"`.
...
- `meta`: Optional dictionary with metadata that you want to attach to all resulting documents.
Can be any custom keys and values.
- `remove_numeric_tables`: Not applicable.
- `valid_languages`: Validate languages from a list of languages specified in the [ISO 639-1]
(https://en.wikipedia.org/wiki/ISO_639-1) format.
...
- `id_hash_keys`: Generate the document ID from a custom list of strings that refer to the document's
attributes. To ensure you don't have duplicate documents in your DocumentStore but texts are
not unique, modify the metadata and pass, for example,  `"meta"` to this field ([`"content"`, `"meta"`]).
If you do this, the ID is generated by using the content and the defined metadata.
