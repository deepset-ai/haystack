...
should be changed to what you have set for self.content_field and self.name_field.
- `index`: OpenSearch index where the documents should be indexed. If not supplied, self.index will be used.
...
fail: an error is raised if the document ID of the document being added already
exists.
- `headers`: Custom HTTP headers to pass to OpenSearch client (e.g. {'Authorization': 'Basic YWRtaW46cm9vdA=='})
Check out https://www.elastic.co/guide/en/elasticsearch/reference/current/http-clients.html for more information.
