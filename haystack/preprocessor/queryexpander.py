...
class QueryExpander:
  ...
  def remove_stop(query,lang = None):
    ...
    if(lang in ["en"]):
        keyword_model = spacy_models[lang]

    elif(lang in ["de","fr","es","pt","it","nl","el","nb","lt"]):
        try:
    ...
    notStopWords = [notStopWords.text for notStopWords in query if not notStopWords.is_stop]
    result = ""
    for xy in range(num_keyword_repeats):
