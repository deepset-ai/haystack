...
def clean(
    ...
    if clean_whitespace:
        ...
        clean_document = replace_regex_matches(document=clean_document, pattern=r"[ \t\r\v]*\f[ \t\r\v]*", string="\f")
        # Whitespace around newlines
        clean_document = replace_regex_matches(document=clean_document, pattern=r"[ \t\r\v]*\n[ \t\r\v]*", string="\n")
...
def replace_regex_matches(document: Document, pattern: str, string: str) -> Document:
    ...
    for headline in document.meta.get("headlines", []):

        # Chec for a match at the start that would force us to shift start_idx rightward
    ...
    for doc in split_docs:
        split_headlines += doc.meta.get("headlines", [])
    if split_headlines:
        document.meta["headlines"] = split_headlines
