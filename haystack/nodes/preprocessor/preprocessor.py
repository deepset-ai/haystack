...
class PreProcessor(BasePreProcessor):
    def __init__(
        ...
        split_regex_pattern: Optional[str] = None,
    ):
        """
        ...
        :param split_by: Unit for splitting the document. Can be "word", "sentence", "passage" or "regex". Set to None to disable splitting.
        ...
        :param split_regex_pattern: If `split_by` is set to `regex`, the actual regex pattern for splitting can be defined here.
        ...
        self.split_regex_pattern = split_regex_pattern
    ...
    def split(
        ...
        if split_respect_sentence_boundary and split_by == "word":
            ...
            indices = [m.start(0) for m in re.finditer(pattern=self.split_regex_pattern, string=text)]
            indices.insert(0, 0)
            text_splits = [text[i:j] for i, j in zip(indices, indices[1:] + [None])]
        else:
