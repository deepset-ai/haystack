...
class PreProcessor(BasePreProcessor):
    def __init__(
        ...
        :param split_regex: If `split_by` is set to `regex`, the actual regex pattern for splitting can be defined here.
    ...
    def split(
        ...
        if split_respect_sentence_boundary and split_by == "word":
            ...
            if split_by == "passage":
                ...
                indices = [m.start(0) for m in re.finditer(pattern=self.split_by_regex, string=text)]
                indices.insert(0, 0)
                elements = [text[i:j] for i, j in zip(indices, indices[1:] + [None])]
            else:
