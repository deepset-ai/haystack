            ...
            if filter_urls:
                ...
                for url in urls:
                    if pattern.search(url):
                        file_paths += self._write_to_files([url], output_dir=output_dir, extract_hidden_text=extract_hidden_text)
            else:
            ...
            if crawler_depth == 1:
                ...
                for url, extracted_sublink in sub_links.items():
                    file_paths += self._write_to_files(
                        extracted_sublink, output_dir=output_dir, base_url=url, id_hash_keys=id_hash_keys, extract_hidden_text=extract_hidden_text
                    )

        return file_paths

    def _write_to_files(
        self, urls: List[str], output_dir: Path, extract_hidden_text: bool, base_url: str = None, id_hash_keys: Optional[List[str]] = None
    ) -> List[Path]:
        paths = []
        for link in urls:
