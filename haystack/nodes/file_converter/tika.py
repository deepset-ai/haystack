...
class TikaConverter(BaseConverter):
    ...
    def convert(
        ...
        :param id_hash_keys: Generate the document id from a custom list of strings that refer to the document's
            ...
            not unique, you can modify the metadata and pass e.g. `"meta"` to this field (e.g. [`"content"`, `"meta"`]).
            In this case the id will be generated by using the content and the defined metadata.
        :param xmlContent: If True, the content is returned as XML. If False, the content is returned as plain text. Tika's default is False, but xml contains more data that can be useful for finding page and paragraph breaks
