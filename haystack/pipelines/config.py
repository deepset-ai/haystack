...
def validate_yaml(path: Path, strict_version: bool = False):
    """
    Validates the given YAML file using the autogenerated JSON schema.

    :param pipeline_config: the configuration to validate
    :param strict_version: whether to fail in case of a version mismatch (throws a warning otherwise)
    :return: None if validation is successful
    :raise: `PipelineConfigError` in case of issues.
    """
    pipeline_config = read_pipeline_config_from_yaml(path)
    validate_config(pipeline_config=pipeline_config, strict_version=strict_version)
    logging.debug(f"'{path}' contains valid Haystack pipelines.")


def validate_config(pipeline_config: Dict, strict_version: bool False) -> None:
    """
    ...
    if not strict_version:
        ...
        try:
            ...
            raise PipelineConfigError(
                ...
                "If you're loading a pipeline from YAML, make sure to have a top-level parameter "
                "called 'version' in your YAML file and specify which Haystack version you're using."
            )
