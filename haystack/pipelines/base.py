...
ALLOWED_INPUT_CHARS = re.compile(r"^[-a-zA-Z0-9_/.:]+$")
...
class BasePipeline(ABC):
    ...
    @classmethod
    def validate_config(cls, pipeline_config: Dict, version: Optional[str] = None) -> None:
        """
        ...
        """
        with open(JSON_SCHEMAS_PATH / f"haystack-pipeline-{version or VERSION}.schema.json", "r") as schema_file:
    ...
    @classmethod
    def validate_yaml(cls, path: Path, version: Optional[str] = None):
        """
        Validates the given configuration using the autogenerated JSON schema for the specified version
        (defaults to the version of the currently installed Haystack).
    ...
    @classmethod
    def _read_pipeline_config_from_yaml(cls, path: Path, version: Optional[str] = None):
        try:
            with open(path, "r", encoding="utf-8") as stream:
                config = yaml.safe_load(stream)
                cls.validate_config(config, version=version or VERSION)

                if config["version"] != VERSION:
                    logger.warning(
                        f"YAML version ({config['version']}) does not match with Haystack version ({VERSION}). "
...
class _PipelineCodeGen:
    ...
    def _validate_user_input(cls, input: str):
        if isinstance(input, str) and not ALLOWED_INPUT_CHARS.match(input):
