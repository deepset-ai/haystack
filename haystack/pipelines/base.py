...
class BasePipeline:
    ...
    @classmethod
    def _remove_nested_keys_from_json_schema(cls, schema: Dict[str, Any], key_to_remove: str) -> Dict:
        """
        Small function to remove keys from the JSON schema that are unsupported by the `jsonschema` package
        """
        ...
        for key, value in schema.items():
            if key != key_to_remove:
                if isinstance(value, dict):
                    ...
                    new_schema[key] = [cls._remove_nested_keys_from_json_schema(v, key_to_remove) for v in value]
                else:
                    new_schema[key] = value
        return new_schema

    @classmethod
    def validate_config(cls, pipeline_config: Dict, version: str = str(__version__)) -> None:
        """
        Validates the given configuration using the autogenerated JSON schema for the specified version
        (defaults to the version of the currently installed Haystack).
    ...
    @classmethod
    def validate_yaml(cls, path: Path, version: str = str(__version__)):
        """
        Validates the given configuration using the autogenerated JSON schema for the specified version
        (defaults to the version of the currently installed Haystack).
