...
class KGQARetriever(BaseGraphRetriever):
    def __init__(self, knowledge_graph):
        ...
        self.subject_names = Counter([result["s"]["value"] for result in self.knowledge_graph.get_all_subjects(index="hp-test")])
        self.predicate_names = Counter([result["p"]["value"] for result in self.knowledge_graph.get_all_predicates(index="hp-test")])
        self.object_names = Counter([result["o"]["value"] for result in self.knowledge_graph.get_all_objects(index="hp-test")])
    ...
    def retrieve(self, question_text: str, top_k_graph: int):
        ...
        results = []
        logger.info(f"Listing top {min(top_k_graph,len(queries_with_scores))} queries (k={top_k_graph}) and answers for question \"{question.question_text}\"")
    ...
    def filter_existing_entities(self):
        ...
        for (alias, v) in self.alias_to_entity_and_prob.items():
            ...
            for (entity, probability) in v:
                if Question.add_namespace_to_resource(entity) in self.subject_names or Question.add_namespace_to_resource(entity) in self.object_names:
    ...
    def query_generation(self, triples: Set[Triple], question_type: QuestionType) -> List[Query]:
        ...
        """
        queries: List[Query] = []
        for k in range(max(len(triples), 4)):
