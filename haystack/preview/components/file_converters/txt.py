...
class TextfileToDocument:
    ...
    def run(self, data):
        ...
        metas = TextfileToDocument._prepare_meta_data(data.meta, file_paths)
        ...
        for path, meta in zip(file_paths, metas):
            ...
            if data.valid_languages is not None:
                if not TextfileToDocument._validate_language(text, data.valid_languages):
                    logger.warning(
                        f"Text from file {path} is not in one of the valid languages: {data.valid_languages}. "
    ...
    @staticmethod
    def _prepare_meta_data(meta: Optional[Union[Dict, List[Dict]]], file_paths: List[Union[str, Path]]) -> List[Dict]:
        """
