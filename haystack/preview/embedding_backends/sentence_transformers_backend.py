...
class SentenceTransformersEmbeddingBackend:
    ...
    _instances: Dict[str, "SentenceTransformersEmbeddingBackend"] = {}

    def __new__(cls, *args, **kwargs):
        args_kwargs_str = str(args) + str(kwargs)
        instance_id = hashlib.md5(args_kwargs_str.encode())
    ...
    def embed(self, data: List[str], *inference_params) -> np.ndarray:
        embedding = self.model.encode(data, *inference_params)
        return embedding
