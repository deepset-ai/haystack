...
"""
    ...
    An example report can be inspected via calling print_telemetry_report(). Check out the documentation for more details: URL
...
class NonPrivateParameters:
    ...
    def apply_filter(cls, param_dicts: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "model_name_or_path" in tracked_params:
            if Path(tracked_params["model_name_or_path"]).is_file():
