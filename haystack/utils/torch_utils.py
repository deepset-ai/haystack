...
def extract_torch_dtype(**kwargs) -> Optional["torch.dtype"]:  # type: ignore
    """
    ...
    if torch_dtype is not None:
        if isinstance(torch_dtype, str):
            if "torch." in torch_dtype:
                ...
                raise ValueError(
                    f"torch_dtype should be a torch.dtype, a string with 'torch.' prefix or the string 'auto', "
                    f"got {torch_dtype}"
                )
        elif isinstance(torch_dtype, torch.dtype):  # pylint: disable=no-member
            torch_dtype_resolved = torch_dtype
        else:
