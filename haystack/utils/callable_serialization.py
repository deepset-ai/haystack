...
def deserialize_callable(callable_handle: str) -> Callable:
    ...
    for i in range(len(parts), 0, -1):
        ...
        for part in parts[i:]:
            ...
            # A crafted handle can walk through an object re-exported from an unallowlisted module
            # and then reach a final callable whose own module is allowlisted. For example, an
            # allowlisted Haystack module re-exports `rich.console.Console`; walking through that
            # class to `Console._environ.update` ends at `collections.abc.MutableMapping.update`,
            # hiding the unallowlisted `rich` hop from the final check below. Validate every object
            # reached during traversal so no intermediate hop can escape the allowlist.
