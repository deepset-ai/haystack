...
class ElasticsearchDocumentStore(BaseElasticsearchDocumentStore):
    ...
    def _do_bulk(
        self, actions: List[Dict], client: "Elasticsearch", refresh: str, headers: Optional[Dict]  # type: ignore[override]
    ):
    ...
    def _client_search(self, index: str, body: Dict, headers: Optional[Dict]) -> Dict:
        return self.client.search(index=index, **body, headers=headers)

    def _client_update(self, index: str, doc_id: str, meta: Dict, headers: Optional[Dict]):
        self.client.update(index=index, id=doc_id, **meta, headers=headers, refresh=self.refresh_type)

    def _client_indices_create(self, index: str, mapping: Dict, headers: Optional[Dict]):
        self.client.indices.create(index=index, **mapping, headers=headers)
