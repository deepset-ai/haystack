...
from haystack.schema import FilterType, Document

logger = logging.getLogger(__name__)


class BaseElasticsearchDocumentStore(SearchEngineDocumentStore):
    """
    Base class implementing the common logic for Elasticsearch 7 and Elasticsearch 8.

    To use Elasticsearch as a document store, import the ElasticsearchDocumentStore from haystack.document_stores.
    """
            ...
            if self.search_fields:
                for search_field in self.search_fields:
                    if search_field in mapping["properties"]:
                        ...
                        self._client_indices_put_mapping(index=index_id, mapping=mapping, headers=headers)
