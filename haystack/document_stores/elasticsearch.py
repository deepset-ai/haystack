...
class ElasticsearchDocumentStore(KeywordDocumentStore):
    def __init__(
        ...
        :param aws4auth: Authentication for usage with AWS Elasticsearch (can be generated with the requests-aws4auth
            package).
        :param index: Name of index in Elasticsearch to use for storing the documents that we want to search.
            If not existing yet, we will create one.
        :param label_index: Name of index in Elasticsearch to use for storing labels. If not existing yet, we will
            create one.
        :param search_fields: Name of fields used by ElasticsearchRetriever to find matches in the docs to our incoming
            query (using Elastic's multi_match query), e.g. `["title", "full_text"]`.
        :param content_field: Name of field that might contain the answer and will therefore be passed to the Reader
            Model (e.g. `"full_text"`). If no Reader is used (e.g. in FAQ-Style QA), the plain content of this field
        ...
        :param embedding_dim: Dimensionality of embedding vector. (Only needed when using a dense retriever
            (e.g. DensePassageRetriever, EmbeddingRetriever) on top.)
        ...
        :param analyzer: Specify the default analyzer from one of the built-ins when creating a new Elasticsearch Index.
            Elasticsearch also has built-in analyzers for different languages (e.g. impacting tokenization). More info
            at: [https://www.elastic.co/guide/en/elasticsearch/reference/7.9/analysis-analyzers.html](https://www.elastic.co/guide/en/elasticsearch/reference/7.9/analysis-analyzers.html)
        :param excluded_meta_data: Name of fields in Elasticsearch that should not be returned
            (e.g. `["field_one", "field_two]"`).
            Helpful if you have fields with long, irrelevant content that you don't want to display in results
        ...
        :param refresh_type: Type of ES refresh used to control when changes made by a request (e.g. bulk) are made
            visible to search.
            If set to `"wait_for",` continue only after changes are visible (slow, but safe).
            If set to `"false"`, continue directly (fast, but sometimes unintuitive behaviour when docs are not
            immediately available after ingestion).
            More info at [https://www.elastic.co/guide/en/elasticsearch/reference/6.8/docs-refresh.html](https://www.elastic.co/guide/en/elasticsearch/reference/6.8/docs-refresh.html).
        ...
        :param scroll: Determines how long the current index is fixed, e.g. during updating all documents with
            embeddings. Defaults to `"1d"` and should not be larger than this. Can also be in minutes `"5m"` or
                hours `"15h"`.
                For details, see [https://www.elastic.co/guide/en/elasticsearch/reference/current/scroll-api.html](https://www.elastic.co/guide/en/elasticsearch/reference/current/scroll-api.html).
        :param skip_missing_embeddings: Parameter to control queries based on vector similarity when indexed documents
            miss embeddings.\

            Parameter options:
                - `True`: Query will ignore all documents without embeddings (default, recommended if you concurrently
                index and query).
        ...
        :param synonyms: List of synonyms can be passed while Elasticsearch initialization.
             ...
             More info at: [https://www.elastic.co/guide/en/elasticsearch/reference/current/analysis-synonym-tokenfilter.html](https://www.elastic.co/guide/en/elasticsearch/reference/current/analysis-synonym-tokenfilter.html).
        :param synonym_type: Synonym filter type.
            Synonym or Synonym_graph to handle synonyms, including multi-word synonyms, correctly during the analysis
            process.
            More info at: [https://www.elastic.co/guide/en/elasticsearch/reference/current/analysis-synonym-graph-tokenfilter.html](https://www.elastic.co/guide/en/elasticsearch/reference/current/analysis-synonym-graph-tokenfilter.html).
        """
    ...
    def _create_document_index(self, index_name: str, headers: Optional[Dict[str, str]] = None):
        """
        Create a new index for storing documents. In case an index with the name already exists, it ensures that
    ...
    def get_metadata_values_by_key(
        ...
        :param filters: Narrow down the scope to documents that match the given filters.
            ...
            operation.
                ...
                ```
        :param index: Elasticsearch index where the meta values should be searched. If not supplied, self.index will be
            used.
        :param headers: Custom HTTP headers to pass to elasticsearch client
            (e.g. `{"Authorization": "Basic YWRtaW46cm9vdA=="}`).
            Check out [https://www.elastic.co/guide/en/elasticsearch/reference/current/http-clients.html](https://www.elastic.co/guide/en/elasticsearch/reference/current/http-clients.html)
    ...
    def write_documents(
        ...
        :param index: Elasticsearch index where the documents should be indexed. If not supplied, self.index will be
            used.
    ...
    def write_labels(
        ...
        :param index: Elasticsearch index where the labels should be stored. If not supplied, self.label_index will be
            used.
        :param batch_size: Number of labels that are processed at a time.
        :param headers: Custom HTTP headers to pass to elasticsearch client
            (e.g. `{"Authorization": "Basic YWRtaW46cm9vdA=="}`).
            Check out [https://www.elastic.co/guide/en/elasticsearch/reference/current/http-clients.html](https://www.elastic.co/guide/en/elasticsearch/reference/current/http-clients.html)
            for more information.
    ...
    def query(
        ...
        query as defined by the BM25 algorithm.
        ...
        :param custom_query: Query string as per Elasticsearch DSL with a mandatory query placeholder(query).
            ...
            names must match with the filters dict supplied in self.retrieve().
                ...
                __For this custom_query, a sample retrieve() could be:__
                ```python
                ...
                See [https://www.elastic.co/guide/en/elasticsearch/reference/current/highlighting.html](https://www.elastic.co/guide/en/elasticsearch/reference/current/highlighting.html).
                You will find the highlighted output in the returned Document's meta field by key `"highlighted"`.

                __Example custom_query with highlighting:__
                ```python
                {
                    "size": 10,
                    "query": {
                    ...
                    "highlight": {"fields": {"content": {}, "title": {}}},  # enable highlighting for fields "content" and "title"
                }
                ```

                __For this custom_query, highlighting info can be accessed by:__
                ```python
    ...
    def update_embeddings(
        ...
        This can be useful if you want to add or change the embeddings for your documents
    ...
    def delete_all_documents(
        ...
        :param index: Index name to delete the documenst from.
...
class OpenSearchDocumentStore(ElasticsearchDocumentStore):
    def __init__(self, verify_certs=False, scheme="https", username="admin", password="admin", port=9200, **kwargs):
        """
        Document Store using OpenSearch ([https://opensearch.org/](https://opensearch.org/)).
        It is compatible with the AWS Elasticsearch Service.
        ...
        :param index: Name of index in OpenSearch to use for storing the documents that we want to search.
            If not existing yet, we will create one.
        :param label_index: Name of index in OPenSearch to use for storing labels. If not existing yet, we will create
            one.
        :param search_fields: Name of fields used by ElasticsearchRetriever to find matches in the docs to our
            incoming query (using OpenSearch's multi_match query), e.g. `["title", "full_text"]`.
        ...
        :param embedding_field: Name of field containing an embedding vector (Only needed when using a dense retriever
            (e.g. DensePassageRetriever, EmbeddingRetriever) on top.)
            Note, that in OpenSearch the similarity type for efficient approximate vector similarity calculations is
            tied to the embedding field's data type which cannot be changed after creation.
        :param embedding_dim: Dimensionality of embedding vector (Only needed when using a dense retriever
            (e.g. DensePassageRetriever, EmbeddingRetriever) on top.)
        :param custom_mapping: If you want to use your own custom mapping for creating a new index in OpenSearch,
            you can supply it here as a dictionary.
        :param analyzer: Specify the default analyzer from one of the built-ins when creating a new OpenSearch Index.
                         OpenSerach also has built-in analyzers for different languages (e.g. impacting tokenization).
                         More info at: [https://opensearch.org/docs/latest/opensearch/query-dsl/full-text/#options](https://opensearch.org/docs/latest/opensearch/query-dsl/full-text/#options).
        :param excluded_meta_data: Name of fields in OpenSearch that should not be returned
            (e.g. `["field_one", "field_two"]`). Helpful if you have fields with long, irrelevant content that you don't
            want to display in results (e.g. embedding vectors).
        :param scheme: `"https"` or `"http"`, protocol used to connect to your elasticsearch instance.
        :param ca_certs: Root certificates for SSL: it is a path to certificate authority (CA) certs on disk.
            You can use certifi package with certifi.where() to find where the CA certs file is located in your machine.
        :param verify_certs: Whether to be strict about ca certificates.
        ...
        :param refresh_type: Type of ES refresh used to control when changes made by a request (e.g. bulk) are made
            ...
            If set to `"false"`, continue directly (fast, but sometimes unintuitive behaviour when docs are not
            immediately available after ingestion).
            More info at [https://opensearch.org/docs/latest/opensearch/rest-api/document-apis/bulk/#url-parameters](https://opensearch.org/docs/latest/opensearch/rest-api/document-apis/bulk/#url-parameters).
        :param similarity: The similarity function used to compare document vectors. `"dot_product"` is the default
            since it is more performant with DPR embeddings. `"cosine"` is recommended if you are using a Sentence BERT
            model. Note, that the use of efficient approximate vector calculations in OpenSearch is tied to
            embedding_field's data type which cannot be changed after creation. You won't be able to use approximate
            vector calculations on an embedding_field which was created with a different similarity value.
        ...
        :param index_type: The type of index to be created. Choose from `"flat"` and `"hnsw"`.
            ...
            similarity calculations, we don't make use of exact vector similarity when `index_type="flat"`. Instead,
            we use the same approximate vector similarity calculations like in `"hnsw"`, but further optimized for
            accuracy. Exact vector similarity is only used as fallback when there's a mismatch between certain requested
            and indexed similarity types. In these cases, however, a warning will be displayed. See similarity param for
            more information.
        :param scroll: Determines how long the current index is fixed, e.g. during updating all documents with
            embeddings. Defaults to `"1d"` and should not be larger than this. Can also be in minutes `"5m"` or hours
            `"15h"`.
            For details, see [https://opensearch.org/docs/latest/opensearch/rest-api/scroll/](https://opensearch.org/docs/latest/opensearch/rest-api/scroll/).
        :param skip_missing_embeddings: Parameter to control queries based on vector similarity when indexed documents
            miss embeddings.\
