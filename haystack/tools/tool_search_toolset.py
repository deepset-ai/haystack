...
class ToolSearchToolset(Toolset):
    ...
    def _create_search_tool(self) -> Tool:
        ...
        def search_tools(query: str, k: int | None = None) -> str:
            ...
            :returns: List of found tools with their descriptions.
            """
            ...
            if not results:
                return f"No tools found matching '{query}'. Try a different search query."

            # Auto-load found tools
            output_lines = [f"Found {len(results)} tool(s) matching '{query}':"]
            for tool, score in results:
                self._discovered_tools[tool.name] = tool
                output_lines.append(f"- {tool.name} (score: {score:.2f}): {tool.description}")
        ...
        return Tool(
            ...
            "'web search', 'database query'). This tool will find relevant tools from the catalog and automatically "
            "load them for your use. You cannot use other tools until you discover and load them with this search.",
            parameters={
                "type": "object",
                "properties": {
