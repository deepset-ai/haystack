...
class OpenAPIServiceConnector:
    ...
    determine the method to be called and the parameters to be passed. The message payload should be an OpenAI
    JSON formatted function calling string consisting of the method name and the parameters to be passed to the method.
    The method name and parameters are then used to invoke the method on the OpenAPI service. The response from the
    service is returned as a ChatMessage.
    ...
    Note however that OpenAPIServiceConnector is usually not meant to be used directly, but rather as part of a pipeline
    that includes the OpenAPIServiceToFunctions component and an OpenAIChatGenerator component using LLM with the
    function calling capabilities. In the example below we use the function calling payload directly, but in a
