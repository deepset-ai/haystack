...
class JsonSchemaValidator:
    ...
    def run(
        ...
        :raises ValueError: If the messages list is empty, if the last message has no text content, or if
            no JSON schema is provided either in the `run` method or in the component init.
        """
        if not messages:
            raise ValueError(
                "The `messages` list is empty; JsonSchemaValidator validates the last message of the list, "
                "so at least one message is required."
            )
