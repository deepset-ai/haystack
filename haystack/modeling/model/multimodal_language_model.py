...
class MultiModalLanguageModel(nn.Module, ABC):
    """
    Parent class for models that can embed data into a **COMPARABLE** semantic vector space.

    These models read feature vectors (generated by a feature extractor) and return vectors
    that capture the meaning of the original data.

    Models inheriting from MultiModalLanguageModel are designed to be used in parallel one with
    the other in multimodal retrieval settings, for example image retrieval from a text query,
    combined table and text retrieval, etc... They must therefore embed their source data into
    comparable vector spaces.

    See Facebook's Data2Vec family of models for an example of models with comparable outputs.
    """
    ...
    def expected_inputs(cls) -> Tuple[Set[str], Set[str]]:
        """
        Returns a tuple, (List[mandatory args names], List[all args names])
        """
        pass
    ...
    def forward(self, **kwargs):
        ...
        """
        mandatory_args, all_args = self.expected_inputs
...
class ImageLanguageModel(MultiModalLanguageModel):
    ...
    @property
    def expected_inputs(cls) -> Tuple[Set[str], Set[str]]:
        return {"pixel_values"}, {"pixel_values", "bool_masked_pos", "head_mask"}
...
def get_mm_language_model(
    ...
    if not config.model_type:
        logger.error(
            f"Model type not understood for '{model_name}'. Either supply the local path for a saved model "
            "or the name of a model that can be downloaded from the Model Hub.\n"
            f"Using the AutoModel class for '{pretrained_model_name_or_path}'. This can cause crashes!"
        )
