...
class StreamingDataSilo:
    ...
    def __init__(self, processor: Processor, batch_size: int, distributed: bool = False, dataloader_workers: int = 8):
        """
...
class _StreamingDataSet(IterableDataset):
    def __init__(self, processor: Processor, filepath: Path, batch_size: int, dataloader_workers: int,
                 distributed: bool = False, n_samples: Optional[int] = None):
        """
        :param processor: A dataset specific Processor object which will turn input file into a Pytorch Dataset.
    ...
    def shuffle_files(self, files: List, seed: Optional[int] = None):
        if not seed:
