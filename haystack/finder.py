...
class Finder:
    ...
    def _eval_counts_reader(self, question: dict, predicted_answers: dict, metric_counts: Counter):
            ...
            for answer_idx, answer in enumerate(predicted_answers["answers"]):
                ...
                if answer["document_id"] == question["correct_es_doc_id"]:
                    ...
                    for gold_span in gold_spans:
                        ...
                        if not found_answer:
                            metric_counts, found_answer = self._count_overlap(
                                gold_span, predicted_span, metric_counts, answer_idx
                            )
                        # check for exact match
                        if not found_em:
                            metric_counts, found_em = self._count_exact_match(
                                gold_span, predicted_span, metric_counts, answer_idx
                            )
                        # calculate f1
                        current_f1 = self._calculate_f1(gold_span, predicted_span)
                        # top-1 answer
                        if answer_idx == 0:
        ...
        # question not answerable
            ...
            metric_counts = self._count_no_answer(predicted_answers["answers"], metric_counts)

        return metric_counts

    def _eval_counts_reader_batch(self, pred: dict, metric_counts: Counter):
        # Calculates evaluation metrics for one question and returns adds results to counter.
        ...
        # question not answerable
            ...
            metric_counts["number_of_no_answer"] += 1
            metric_counts = self._count_no_answer(pred["answers"], metric_counts)
    ...
    def _count_exact_match(self, gold_span: (int, int), predicted_span: (int, int), metric_counts: Counter, answer_idx: int):
