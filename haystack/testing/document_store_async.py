...
class GetMetadataFieldUniqueValuesAsyncTest:
    ...
    @staticmethod
    @pytest.mark.asyncio
    async def test_get_metadata_field_unique_values_type_fidelity_async(document_store: AsyncDocumentStore):
        """
        Test get_metadata_field_unique_values_async() preserves each value's original type per field.

        int, float, str and bool are each stored under their own metadata field. Unlike
        test_get_metadata_field_unique_values_distinct_types_async, this doesn't require a single field
        to hold multiple types at once - some backends can't do that (their field type is fixed by the
        ...
        int_values, int_count = await document_store.get_metadata_field_unique_values_async(  # type:ignore[attr-defined]
            metadata_field="distinct_type_int"
        )
        str_values, str_count = await document_store.get_metadata_field_unique_values_async(  # type:ignore[attr-defined]
            metadata_field="distinct_type_str"
        )
        float_values, float_count = await document_store.get_metadata_field_unique_values_async(  # type:ignore[attr-defined]
            metadata_field="distinct_type_float"
        )
        bool_values, bool_count = await document_store.get_metadata_field_unique_values_async(  # type:ignore[attr-defined]
