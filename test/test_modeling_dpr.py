...
def test_dpr_processor_save_load(tmpdir):
    d = {
        "query": "big little lies season 2 how many episodes ?",
        "passages": [
            {
    ...
    processor.save(save_dir=f"{tmpdir}/testsave/dpr_processor")
    dataset, tensor_names, _ = processor.dataset_from_dicts(dicts=[d], return_baskets=False)
    loadedprocessor = TextSimilarityProcessor.load_from_dir(load_dir=f"{tmpdir}/testsave/dpr_processor")
...
def test_dpr_processor_save_load_non_bert_tokenizer(tmpdir, query_and_passage_model):
    """
    ...
    save_dir = f"{tmpdir}/testsave/dpr_model"
