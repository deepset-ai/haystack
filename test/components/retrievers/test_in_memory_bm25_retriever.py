...
class TestMemoryBM25Retriever:
    ...
    @pytest.mark.asyncio
    async def test_run_async_with_filter_policy_merge_does_not_leak_filters_between_runs(self, in_memory_doc_store):
        in_memory_doc_store.write_documents(
            [
                Document(content="python article", meta={"tenant": "a", "kind": "article", "year": 2019}),
                Document(content="python blog", meta={"tenant": "a", "kind": "blog", "year": 2019}),
        ...
        second_result = await retriever.run_async(
            query="python", filters={"field": "meta.year", "operator": "==", "value": 2019}
        )
