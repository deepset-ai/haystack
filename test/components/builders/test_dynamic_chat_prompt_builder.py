...
class TestDynamicChatPromptBuilder:
    def test_initialization(self):
        ...
        assert set(builder.__haystack_inputs__._sockets_dict.keys()) == expected_keys

        # response is always prompt regardless of chat mode
        assert set(builder.__haystack_outputs__._sockets_dict.keys()) == {"prompt"}

        # prompt_source is a list of ChatMessage
        assert builder.__haystack_inputs__._sockets_dict["prompt_source"].type == List[ChatMessage]

        # output is always prompt, but the type is different depending on the chat mode
        assert builder.__haystack_outputs__._sockets_dict["prompt"].type == List[ChatMessage]
