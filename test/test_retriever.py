...
def test_dpr_saving_and_loading(tmpdir, retriever, document_store):
    retriever.save(f"{tmpdir}/test_dpr_save")
    ...
    loaded_retriever = DensePassageRetriever.load(f"{tmpdir}/test_dpr_save", document_store)
...
@pytest.mark.parametrize("retriever", ["table_text_retriever"], indirect=True)
@pytest.mark.embedding_dim(512)
def test_table_text_retriever_saving_and_loading(tmpdir, retriever, document_store):
    retriever.save(f"{tmpdir}/test_table_text_retriever_save")
    ...
    loaded_retriever = TableTextRetriever.load(f"{tmpdir}/test_table_text_retriever_save", document_store)
