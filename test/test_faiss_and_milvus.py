...
def test_delete_docs_by_id(document_store, retriever):
    ...
    document_store.update_embeddings(retriever=retriever, batch_size=4)
    assert document_store.get_embedding_count() == 6
    doc_ids = [doc.id for doc in document_store.get_documents()]
...
def test_delete_docs_by_id_with_filters(document_store, retriever):
    ...
    ids_to_delete = [doc.id for doc in document_store.get_documents(filters={"name": ["name_1", "name_2"]})]
    ids_not_to_delete = [doc.id for doc in document_store.get_documents(filters={"name": ["name_3", "name_4", "name_5", "name_6"]})]
    ...
    assert all(doc.meta["meta_field"] != "name_1" for doc in documents)
    assert all(doc.meta["meta_field"] != "name_2" for doc in documents)
