...
def run_single_retriever(
    ...
    retriever = index_documents(
        documents=documents, document_store=document_store, retriever_name=retriever_name, max_seq_len=max_seq_len
    )
    ...
    if retriever_name in ["bm25", "doc2query"]:
        for idx, query in enumerate(queries):
            ...
            convert_and_save_result(
                query_id=query_id, result=result, filename=result_filename_part, save_path=result_output_path
            )
    else:
    ...
    with open(logfile, "a") as f:
        f.write(
            json.dumps({"retriever": retriever_name.split("/")[-1], "duration": stop_time, "start": start_time}) + "\n"
        )


def run_single_dataset_retrieval(retrievers, result_base_path, run_name, dataset_name, top_k, logfile):
...
def run_dataset_retrievals(
    datasets, retriever_names, emb_dims, max_seq_lens, top_k, logfile, result_base_path, run_name
):
