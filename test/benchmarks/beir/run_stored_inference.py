...
def run_stored_inference(
    ...
    pipeline = Pipeline()
    pipeline = init_result_loader_nodes(
        retriever_names=retriever_names, pipeline=pipeline, stored_results=stored_results
    )
    ...
    ##### Haystack Evaluation ######
    ################################
    result = pipeline.eval(labels=labels, params={"Joiner": {"top_k_join": join_top_k}, **retriever_params})
    ...
    for label in labels:
        results = pipeline.run(
            query=label.query, labels=label, params={"Joiner": {"top_k_join": join_top_k}, **retriever_params}
        )
    ...
    for retriever_name in retriever_names:
        ...
        ndcg_single, _map, _recall, _precision = EvaluateRetrieval.evaluate(
            qrels, qrel_retriever_results[retriever_name], [top_k]
        )
