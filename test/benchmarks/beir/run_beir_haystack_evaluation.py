...
from haystack.nodes import EmbeddingRetriever, ElasticsearchRetriever, JoinDocuments, QuestionGenerator, Doc2QueryExpander
...
def evaluate_ensembled_pipeline(dataset_name, dataset_path, retriever_name, top_k, max_seq_len, emb_dim, es_launch=False, recreate_index=True):
    ...
    retriever = index_documents(documents, document_store, retriever_name=retriever_name, max_seq_len=max_seq_len, recreate_index=recreate_index)
    retriever_bm25 = index_documents(documents, document_store, retriever_name='bm25', max_seq_len=max_seq_len, recreate_index=False)
