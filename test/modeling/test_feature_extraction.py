...
class TestFeatureExtraction:
    ...
    def test_save_load_compare(self, tmp_path, model_name: str):
    ...
    @pytest.mark.parametrize("model_name", TOKENIZERS_TO_TEST)
    def test_tokenization_on_edge_cases_full_sequence_tokenization(self, model_name: str, edge_case: str):
        """
        Verify that tokenization on full sequence is the same as the one on "whitespace tokenized words"
        """
        tokenizer = FeatureExtractor(
            pretrained_model_name_or_path=model_name, do_lower_case=False, add_prefix_space=True
        )
        ...
        expected_tokenization = tokenizer(" ".join(edge_case.split()))  # remove multiple whitespaces
    ...
    def test_tokenization_on_edge_cases_full_sequence_verify_spans(self, model_name: str, marker: str, edge_case: str):
        tokenizer = FeatureExtractor(
            pretrained_model_name_or_path=model_name, do_lower_case=False, add_prefix_space=True
        )
    ...
    def test_detokenization_for_bert(self, edge_case):
    ...
    @pytest.mark.end2end
    def test_tokenize_custom_vocab_bert(self):
        tokenizer = FeatureExtractor(pretrained_model_name_or_path=BERT, do_lower_case=False)
