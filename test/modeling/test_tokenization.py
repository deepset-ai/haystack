...
def test_basic_loading(caplog):
    ...
    # slow tokenizers
    tokenizer = get_tokenizer(pretrained_model_name_or_path="bert-base-cased", do_lower_case=True, use_fast=False)
    assert type(tokenizer) == BertTokenizer
    assert tokenizer.basic_tokenizer.do_lower_case == True

    tokenizer = get_tokenizer(pretrained_model_name_or_path="xlnet-base-cased", do_lower_case=True, use_fast=False)
    assert type(tokenizer) == XLNetTokenizer
    assert tokenizer.do_lower_case == True

    tokenizer = get_tokenizer(pretrained_model_name_or_path="roberta-base", use_fast=False)
    assert type(tokenizer) == RobertaTokenizer

    # fast tokenizers
    tokenizer = get_tokenizer(pretrained_model_name_or_path="bert-base-cased", do_lower_case=True)
    assert type(tokenizer) == BertTokenizerFast
    assert tokenizer.do_lower_case == True

    tokenizer = get_tokenizer(pretrained_model_name_or_path="xlnet-base-cased", do_lower_case=True)
    assert type(tokenizer) == XLNetTokenizerFast
    assert tokenizer.do_lower_case == True

    tokenizer = get_tokenizer(pretrained_model_name_or_path="roberta-base")
...
def test_save_load(tmp_path, caplog):
    ...
    for lang_name in lang_names:
        if "xlnet" in lang_name.lower():
            t = get_tokenizer(lang_name, lower_case=False, use_fast=True, from_slow=True)
        else:
            t = get_tokenizer(lang_name, lower_case=False)
    ...
    for tokenizer in tokenizers:
        ...
        tokenizer_loaded = get_tokenizer(save_dir, tokenizer_class=tokenizer_type)
...
def test_all_tokenizer_on_special_cases(caplog):
    ...
    for lang_name in lang_names:
        ...
        t = get_tokenizer(lang_name, lower_case=False, add_prefix_space=add_prefix_space)
        tokenizers.append(t)
