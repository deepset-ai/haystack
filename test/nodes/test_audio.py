...
def test_answer_to_speech(tmp_path):
    ...
    answer2speech = AnswerToSpeech(generated_audio_path=tmp_path/"test_audio", audio_naming_function=lambda text: text, transformers_params={"always_fix_seed": True})
...
def test_document_to_speech(tmp_path):
    text_doc = Document(content="this is the content of the document", content_type="text", meta={"name": "test_document.txt"})
    expected_audio_content = SAMPLES_PATH / "audio" / "this is the content of the document.wav"

    tmp_path = Path(__file__).parent

    doc2speech = DocumentToSpeech(generated_audio_path=tmp_path/"test_audio", audio_naming_function=lambda text: text, transformers_params={"always_fix_seed": True})
