...
def test_openai_answer_generator_pipeline_max_tokens():
    ...
    mock_method = create_autospec(openai_generator.run)
    mock_method.return_value = ({"answers": mocked_response}, "output_1")

    with patch.object(openai_generator, "run", new=mock_method) as openai_generator_mock_run_call:
        result = pipeline.run(query=question, documents=nyc_docs, params={"generator": {"max_tokens": 3}})
        assert result["answers"] == mocked_response
        openai_generator_mock_run_call.assert_called_with(query=question, documents=nyc_docs, max_tokens=3)
