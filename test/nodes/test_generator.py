...
class TestLFQA:
    ...
    def test_lfqa_pipeline(self, document_store, retriever, lfqa_generator, docs_with_true_emb):
    ...
    @pytest.mark.embedding_dim(128)
    def test_lfqa_pipeline_unknown_converter(self, document_store, retriever, docs_with_true_emb):
        # reuse existing DOCS but regenerate embeddings with retribert
        docs: List[Document] = []
        for d in docs_with_true_emb:
            docs.append(Document(content=d.content))
        ...
        with pytest.raises(Exception) as exception_info:
            output = pipeline.run(query=query, params={"top_k": 1})
        assert "doesn't have input converter registered for patrickvonplaten/t5-tiny-random" in str(
    ...
    @pytest.mark.parametrize("retriever", ["retribert"], indirect=True)
    @pytest.mark.embedding_dim(128)
    def test_lfqa_pipeline_invalid_converter(self, document_store, retriever, docs_with_true_emb):
        # reuse existing DOCS but regenerate embeddings with retribert
        docs: List[Document] = []
        for d in docs_with_true_emb:
            docs.append(Document(content=d.content))
