...
def test_generator_pipeline_with_translator(
    document_store, retriever, rag_generator, en_to_de_translator, de_to_en_translator, docs_with_embeddings
):
...
@pytest.mark.integration
@pytest.mark.generator
def test_rag_token_generator(rag_generator, docs_with_embeddings):
...
def test_generator_pipeline(document_store, retriever, rag_generator, docs_with_embeddings):
    document_store.write_documents(docs_with_embeddings)
    query = "What is capital of the Germany?"
...
def test_lfqa_pipeline(document_store, retriever, docs_with_embeddings, lfqa_generator):
...
@pytest.mark.embedding_dim(128)
def test_lfqa_pipeline_unknown_converter(document_store, retriever, docs_with_embeddings):
    # reuse existing DOCS but regenerate embeddings with retribert
    docs: List[Document] = []
    for idx, d in enumerate(docs_with_embeddings):
        docs.append(Document(d.content, str(idx)))
...
@pytest.mark.parametrize("retriever", ["retribert"], indirect=True)
@pytest.mark.embedding_dim(128)
def test_lfqa_pipeline_invalid_converter(document_store, retriever, docs_with_embeddings):
    # reuse existing DOCS but regenerate embeddings with retribert
    docs: List[Document] = []
    for idx, d in enumerate(docs_with_embeddings):
        docs.append(Document(d.content, str(idx)))
