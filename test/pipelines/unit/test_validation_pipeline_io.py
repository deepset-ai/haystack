...
def test_find_pipeline_input_one_input():
    ...
    assert _find_pipeline_inputs(pipe.graph) == {
        "comp1": [InputSocket(name="value", type=int, default=inspect.Parameter.empty)],
        "comp2": [],
    }
...
def test_find_pipeline_input_two_inputs_same_component():
    ...
    assert _find_pipeline_inputs(pipe.graph) == {
        "comp1": [
            InputSocket(name="value", type=int, default=inspect.Parameter.empty),
            InputSocket(name="add", type=Optional[int], default=None),
        ],
        "comp2": [],
    }
