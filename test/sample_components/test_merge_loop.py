...
class TestMergeLoop(BaseTestComponent):
    ...
    #         MergeLoop.create(expected_type=int, inputs=["in_1", "in_2"]), tmp_path
    #     )
    ...
    def test_merge_first(self):
        component = MergeLoop.create(expected_type=int, inputs=["in_1", "in_2"])
    ...
    def test_merge_nones(self):
        component = MergeLoop.create(expected_type=int, inputs=["in_1", "in_2", "in_3"])
        results = component.run()
        assert results == {"value": None}
        assert component.init_parameters == {"expected_type": int, "inputs": ["in_1", "in_2", "in_3"]}

    def test_merge_one(self):
        component = MergeLoop.create(expected_type=int, inputs=["in_1"])
    ...
    def test_merge_one_none(self):
        component = MergeLoop.create(expected_type=int, inputs=[])
        results = component.run()
        assert results == {"value": None}
