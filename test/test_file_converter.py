...
def test_convert(Converter):
    converter = Converter()
    document = converter.convert(file_path=Path(__file__).parent/"samples"/"pdf"/"sample_pdf_1.pdf")[0]
...
def test_docx_converter():
    ...
    document = converter.convert(file_path=Path(__file__).parent/"samples"/"docx"/"sample_docx.docx")[0]
    assert document["content"].startswith("Sample Docx File")


def test_markdown_converter():
    converter = MarkdownConverter()
    document = converter.convert(file_path=Path(__file__).parent/"samples"/"markdown"/"sample.md")[0]
