from haystack.utils.url_validation import is_valid_url


def test_is_valid_url_with_valid_http_url():
    url = "http://example.com"
    assert is_valid_url(url)


def test_is_valid_url_with_valid_https_url():
    url = "https://example.com"
    assert is_valid_url(url)


def test_is_valid_url_with_invalid_scheme():
    url = "ftp://example.com"
    assert not is_valid_url(url)


def test_is_valid_url_with_no_scheme():
    url = "example.com"
    assert not is_valid_url(url)


def test_is_valid_url_with_no_netloc():
    url = "http://"
    assert not is_valid_url(url)


def test_is_valid_url_with_empty_string():
    url = ""
    assert not is_valid_url(url)
