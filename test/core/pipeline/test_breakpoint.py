...
class TestSnapshotCallback:
    ...
    def test_trigger_break_point_with_callback(self):
        captured_snapshots = []

        def custom_callback(snapshot: PipelineSnapshot) -> str:
            captured_snapshots.append(snapshot)
            return "triggered_callback"

        snapshot = _create_pipeline_snapshot(
            inputs={},
            component_inputs={},
            break_point=Breakpoint(component_name="comp2", snapshot_file_path="/some/path"),
            component_visits={"comp1": 1, "comp2": 0},
            original_input_data={},
            ordered_component_names=["comp1", "comp2"],
            include_outputs_from=set(),
        ...
        with pytest.raises(BreakpointException) as exc_info:
            _trigger_break_point(pipeline_snapshot=snapshot, snapshot_callback=custom_callback)

        assert len(captured_snapshots) == 1
        assert exc_info.value.pipeline_snapshot_file_path == "triggered_callback"
